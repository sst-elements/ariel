...
enum ArielPageMappingPolicy { LINEAR, RANDOMIZED };

/* Base class for memory managers that cache translation addresses */
class ArielMemoryManagerCache : public ArielMemoryManager {

public:
  /* ELI defines for ArielMemoryManagerCache subcomponents */
#define ARIEL_ELI_MEMMGR_CACHE_PARAMS                                          \
  {"verbose",                                                                  \
   "Verbosity for debugging. Increased numbers for increased verbosity.",      \
   "0"},                                                                       \
      {"vtop_translate",                                                       \
       "Set to yes to perform virt-phys translation (TLB) or no to disable",   \
       "yes"},                                                                 \
      {"pagemappolicy",                                                        \
       "Select the page mapping policy for Ariel [LINEAR|RANDOMIZED]",         \
       "LINEAR"},                                                              \
  {                                                                            \
    "translatecacheentries",                                                   \
        "Keep a translation cache of this many entries to improve emulated "   \
        "core performance",                                                    \
        "4096"                                                                 \
  }

#define ARIEL_ELI_MEMMGR_CACHE_STATS                                           \
  {"tlb_hits", "Hits in the simple Ariel TLB", "hits", 2},                     \
      {"tlb_evicts", "Number of evictions in the simple Ariel TLB",            \
       "evictions", 2},                                                        \
      {"tlb_translate_queries", "Number of TLB translations performed",        \
       "translations", 2},                                                     \
      {"tlb_shootdown", "Number of TLB clears because of page-frees",          \
       "shootdowns", 2},                                                       \
      {"tlb_page_allocs", "Number of pages allocated by the memory manager",   \
       "pages", 2}
   ...
   */
  ArielMemoryManagerCache(ComponentId_t id, Params &params)
    ...
    statTranslationQueries =
        registerStatistic<uint64_t>("tlb_translate_queries");
    ...
    std::string mappingPolicy =
        params.find<std::string>("pagemappolicy", "linear");
    ...
    if (mappingPolicy == "LINEAR" || mappingPolicy == "linear") {
      ...
      output->fatal(
          CALL_INFO, -8,
          "Ariel memory manager - unknown page mapping policy \"%s\"\n",
          mappingPolicy.c_str());
    }
    ...
    translationCacheEntries =
        (uint32_t)params.find<uint32_t>("translatecacheentries", 4096);
...
protected:
  ...
  void mapPagesLinear(uint64_t pageCount, uint64_t pageSize, uint64_t startAddr,
                      std::deque<uint64_t> *freePagePool) {
  ...
  void mapPagesRandom(uint64_t pageCount, uint64_t pageSize, uint64_t startAddr,
                      std::deque<uint64_t> *freePagePool) {
    output->verbose(CALL_INFO, 2, 0,
                    "Page mapping policy is RANDOMIZED map...\n");
    ...
    for (uint64_t j = 0; j < (pageCount * 2); ++j) {
      const uint64_t swapLeft =
          (pageRandomizer.generateNextUInt64() % pageCount);
      const uint64_t swapRight =
          (pageRandomizer.generateNextUInt64() % pageCount);
    ...
    for (uint64_t j = 0; j < pageCount; ++j) {
      output->verbose(CALL_INFO, 64, 0,
                      "Page[%" PRIu64 "] Physical Start=%" PRIu64 "\n", j,
                      preRandomizedPages[j]);
      freePagePool->push_back(preRandomizedPages[j]);
    }
  }

  void populatePageTable(std::string popFilePath,
    ...
    while (!feof(popFile)) {
      ...
      if (freePagePool->size() == 0) {
        output->fatal(CALL_INFO, -1,
                      "Attempted to pin address %" PRIu64
                      " but no free pages.\n",
                      pinAddr);
      }
      ...
      if (pinAddr % pageSize > 0) {
        output->fatal(CALL_INFO, -1,
                      "Attempted to pin address %" PRIu64
                      " but address is not page aligned to page size %" PRIu64
                      "\n",
                      pinAddr, pageSize);
      }
      ...
      output->verbose(CALL_INFO, 4, 0,
                      "Pinning address %" PRIu64 " (physical=%" PRIu64 "\n",
  ...
  void cacheTranslation(uint64_t virtualA, uint64_t physicalA) {
    // Remove the oldest entry if we do not have enough slots TODO is begin()
    // really the oldest...?
    ...
    translationCache->insert(
        std::pair<uint64_t, uint64_t>(virtualA, physicalA));
  }
};
