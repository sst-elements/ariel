...
public:
  ...
  ArielMemoryManager(ComponentId_t id, Params &params) : SubComponent(id) {
    ...
    output = new SST::Output("ArielMemoryManager[@f:@l:@p] ", verbosity, 0,
                             SST::Output::STDOUT);
  }
  ...
  virtual uint32_t getDefaultPool() { return 0; }
  ...
  virtual bool allocateMalloc(const uint64_t size, const uint32_t level,
                              const uint64_t virtualAddress,
                              const uint64_t instructionPointer,
                              const uint32_t thread) {
    output->verbose(CALL_INFO, 4, 0,
                    "The instantiated ArielMemoryManager does not support "
                    "malloc handling.\n");
    return false;
  }
  ...
  virtual bool allocateMMAP(const uint64_t size, const uint32_t level,
                            const uint64_t virtualAddress,
                            const uint64_t instructionPointer,
                            const uint32_t file, const uint32_t thread) {
    output->verbose(CALL_INFO, 4, 0,
                    "The instantiated ArielMemoryManager does nto support MMAP "
                    "handling.\n");
    return false;
  }
  ...
  virtual void freeMMAP(const uint32_t file) {
    output->verbose(CALL_INFO, 4, 0,
                    "The instantiated ArielMemoryManager does not support MMAP "
                    "handling.\n");
  }
  ...
  /* Register handler */
  virtual void registerInterruptHandler(uint32_t core,
                                        InterruptHandlerBase *handler) {
