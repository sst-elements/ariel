...
// General
KNOB<string> SSTNamedPipe(KNOB_MODE_WRITEONCE, "pintool", "p", "",
                          "Named pipe to connect to SST simulator");
KNOB<UINT32> SSTVerbosity(KNOB_MODE_WRITEONCE, "pintool", "v", "0",
                          "SST verbosity level");
KNOB<UINT32> MaxCoreCount(KNOB_MODE_WRITEONCE, "pintool", "c", "1",
                          "Maximum core count to use for data pipes.");
KNOB<UINT32>
    StartupMode(KNOB_MODE_WRITEONCE, "pintool", "s", "1",
                "Mode for configuring profile behavior, 1 = start enabled, 0 = "
                "start disabled, 2 = attempt auto detect");
// Instrumentation control
KNOB<UINT32> InstrumentInstructions(KNOB_MODE_WRITEONCE, "pintool", "E", "1",
                                    "Enable instruction instrumentation");
KNOB<UINT32>
    PerformWriteTrace(KNOB_MODE_WRITEONCE, "pintool", "w", "0",
                      "Perform write tracing (i.e copy values directly into "
                      "SST memory operations) (0 = disabled, 1 = enabled)");
KNOB<UINT32>
    TrapFunctionProfile(KNOB_MODE_WRITEONCE, "pintool", "t", "0",
                        "Function profiling level (0 = disabled, 1 = enabled)");
// Memory/malloc/etc. tracking
KNOB<UINT32> InterceptMemAllocations(
    KNOB_MODE_WRITEONCE, "pintool", "m", "1",
    "Should intercept multi-level memory allocations, mallocs, and frees, 1 = "
    "start enabled, 0 = start disabled");
KNOB<string>
    UseMallocMap(KNOB_MODE_WRITEONCE, "pintool", "u", "",
                 "Should intercept ariel_malloc_flag() and interpret using a "
                 "malloc map: specify filename or leave blank for disabled");
KNOB<UINT32> KeepMallocStackTrace(KNOB_MODE_WRITEONCE, "pintool", "k", "1",
                                  "Should keep shadow stack and dump on malloc "
                                  "calls. 1 = enabled, 0 = disabled");
KNOB<UINT32> DefaultMemoryPool(KNOB_MODE_WRITEONCE, "pintool", "d", "0",
                               "Default SST Memory Pool");
// GPGPUSim
KNOB<string> SSTNamedPipe2(KNOB_MODE_WRITEONCE, "pintool", "g", "",
                           "Named pipe to connect to SST simulator");
KNOB<string> SSTNamedPipe3(KNOB_MODE_WRITEONCE, "pintool", "x", "",
                           "Named pipe to connect to SST simulator");
...
struct mallocFlagInfo {
  ...
  int level;
  int id;
  mallocFlagInfo(bool a, int b, int c, int d)
      : valid(a), count(b), level(c), id(d) {}
};
...
public:
  StackRecord(ADDRINT sp, ADDRINT targ, ADDRINT ip)
      : stackPtr(sp), target(targ), instPtr(ip) {}
...
VOID ariel_stack_call(THREADID thr, ADDRINT stackPtr, ADDRINT target,
                      ADDRINT ip) {
  // Handle longjmp
  while (arielStack[thr].size() > 0 &&
         stackPtr >= arielStack[thr].back().getStackPtr()) {
    // fprintf(btfiles[thr], "RET ON CALL %s (0x%" PRIx64 ", 0x%" PRIx64 ")\n",
    // RTN_FindNameByAddress(arielStack[thr].back().getTarget()).c_str(),
    // arielStack[thr].back().getInstPtr(),
    // arielStack[thr].back().getStackPtr());
...
VOID ariel_stack_return(THREADID thr, ADDRINT stackPtr) {
  ...
  // RTN_FindNameByAddress(arielStack[thr].back().getTarget()).c_str(),
  // arielStack[thr].back().getInstPtr(), arielStack[thr].back().getStackPtr());
  arielStack[thr].pop_back();
}

/* Function to print stack, called by malloc instrumentation code */
VOID ariel_print_stack(UINT32 thr, UINT64 allocSize, UINT64 allocAddr,
                       UINT64 allocIndex) {
  ...
  for (vector<StackRecord>::reverse_iterator rit = arielStack[thr].rbegin();
       rit != arielStack[thr].rend(); rit++) {
    ...
    BT_PRINTF("0x%" PRIx64 ",0x%" PRIx64 ",%s", rit->getTarget(),
              rit->getInstPtr(), ((depth == 0) ? "\n" : ""));
    depth--;
  }
  // Generate any new mappings
  for (std::vector<ADDRINT>::iterator it = newMappings.begin();
       it != newMappings.end(); it++) {
    string file;
    int line;
    PIN_LockClient();
  ...
  for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
    ...
    if (INS_IsCall(tail)) {
      if (INS_IsDirectBranchOrCall(tail)) {
        ...
        INS_InsertPredicatedCall(tail, IPOINT_BEFORE, (AFUNPTR)ariel_stack_call,
                                 IARG_THREAD_ID, IARG_REG_VALUE, REG_STACK_PTR,
                                 IARG_ADDRINT, target, IARG_INST_PTR, IARG_END);
    ...
    if (RTN_Valid(rtn) && ".plt" == SEC_Name(RTN_Sec(rtn))) {
      INS_InsertCall(tail, IPOINT_BEFORE, (AFUNPTR)ariel_stack_call,
                     IARG_THREAD_ID, IARG_REG_VALUE, REG_STACK_PTR,
                     IARG_BRANCH_TARGET_ADDR, IARG_INST_PTR, IARG_END);
    }

    if (INS_IsRet(tail)) {
      INS_InsertPredicatedCall(tail, IPOINT_BEFORE, (AFUNPTR)ariel_stack_return,
                               IARG_THREAD_ID, IARG_REG_VALUE, REG_STACK_PTR,
                               IARG_END);
    }
  }
...
VOID Fini(INT32 code, VOID *v) {
  ...
  if (funcProfileLevel > 0) {
    ...
    for (std::map<std::string, ArielFunctionRecord *>::iterator funcItr =
             funcProfile.begin();
...
VOID WriteFlushInstructionMarker(UINT32 thr, ADDRINT ip, ADDRINT vaddr) {
  ArielCommand ac;
  ac.command =
      ARIEL_FLUSHLINE_INSTRUCTION; /*  Send the Flush instruction commmand */
...
VOID WriteInstructionWrite(ADDRINT *address, UINT32 writeSize, THREADID thr,
  ...
  if (writeTrace) {
    ...
    //          fprintf(stderr, "Error: Payload exceeds maximum size (%d >
    //          %d)\n",
    //              writeSize, ARIEL_MAX_PAYLOAD_SIZE);
    //          exit(-1);
    //      }
    PIN_SafeCopy(&ac.inst.payload[0], address,
                 ARIEL_MIN(writeSize, ARIEL_MAX_PAYLOAD_SIZE));
  }
  /*
...
VOID WriteInstructionReadWrite(THREADID thr, ADDRINT *readAddr, UINT32 readSize,
  ...
  if (enable_output) {
    if (thr < core_count) {
      ...
      WriteInstructionWrite(writeAddr, writeSize, thr, ip, instClass,
                            simdOpWidth);
...
VOID WriteInstructionWriteOnly(THREADID thr, ADDRINT *writeAddr,
                               UINT32 writeSize, ADDRINT ip, UINT32 instClass,
                               UINT32 simdOpWidth) {
...
VOID InstrumentInstruction(INS ins, VOID *v) {
  ...
  if (INS_IsMemoryRead(ins) && INS_IsMemoryWrite(ins)) {
    INS_InsertPredicatedCall(
        ins, IPOINT_BEFORE, (AFUNPTR)WriteInstructionReadWrite, IARG_THREAD_ID,
        IARG_MEMORYREAD_EA, IARG_UINT32, INS_MemoryReadSize(ins),
        IARG_MEMORYWRITE_EA, IARG_UINT32, INS_MemoryWriteSize(ins),
        IARG_INST_PTR, IARG_UINT32, instClass, IARG_UINT32, simdOpWidth,
        IARG_END);
  } else if (INS_IsMemoryRead(ins)) {
    INS_InsertPredicatedCall(
        ins, IPOINT_BEFORE, (AFUNPTR)WriteInstructionReadOnly, IARG_THREAD_ID,
        IARG_MEMORYREAD_EA, IARG_UINT32, INS_MemoryReadSize(ins), IARG_INST_PTR,
        IARG_UINT32, instClass, IARG_UINT32, simdOpWidth, IARG_END);
  } else if (INS_IsMemoryWrite(ins)) {
    INS_InsertPredicatedCall(
        ins, IPOINT_BEFORE, (AFUNPTR)WriteInstructionWriteOnly, IARG_THREAD_ID,
        IARG_MEMORYWRITE_EA, IARG_UINT32, INS_MemoryWriteSize(ins),
        IARG_INST_PTR, IARG_UINT32, instClass, IARG_UINT32, simdOpWidth,
        IARG_END);
  } else {
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)WriteNoOp,
  ...
  if (funcProfileLevel > 0) {
    ...
    if (checkExists == funcProfile.end()) {
      ...
      funcProfile.insert(
          std::pair<std::string, ArielFunctionRecord *>(rtn_name, funcRecord));
    } else {
      funcRecord = checkExists->second;
    }

    INS_InsertPredicatedCall(ins, IPOINT_BEFORE,
                             (AFUNPTR)IncrementFunctionRecord, IARG_PTR,
                             (void *)funcRecord, IARG_END);
  }
}
...
void mapped_ariel_enable() {
  ...
  // Note
  // By adding clock offset calculation, this function now has visible
  // side-effects when called more than once In most cases won't matter ->
  // ariel_enable() called once or close together in time so offsets will
  // stabilize quickly In some cases could cause a big jump in time in the
  // middle of simulation -> ariel_enable() left in app but mode is always-on
  ...
  fprintf(stderr,
          "ARIEL: Enabling memory and instruction tracing from program control "
          "at simulated Ariel cycle %" PRIu64 ".\n",
          tunnel->getCycles());
  fflush(stdout);
  fflush(stderr);
}
...
uint64_t mapped_ariel_cycles() { return tunnel->getCycles(); }

/*
 * Override gettimeofday to return simulated time
 * If ariel_enable is false, returns system gettimeofday value
 * If ariel_enable is true, returns system gettimeofday when ariel was enabled +
 * elapsed simulated time since ariel was enabled
 */
 ...
 * If ariel_enable is true, returns elapsed simulated time since ariel was
 * enabled + clock_gettime(CLOCK_MONOTONIC) from when ariel was enabled
 */
#if !defined(__APPLE__)
...
// same effect as mapped_ariel_output_stats(), but it also sends a user-defined
...
void mapped_ariel_output_stats_buoy(uint64_t marker) {
  ...
  ac.command = ARIEL_OUTPUT_STATS;
  ac.instPtr = (uint64_t)
      marker; // user the instruction pointer slot to send the marker number
  tunnel->writeMessage(thr, ac);
}
...
void ariel_mlm_set_pool(int new_pool) {
  ...
  // #ifdef ARIEL_DEBUG
  //     fprintf(stderr, "Requested: %llu, but expanded to: %llu (on thread:
  //     %lu) \n", size, real_req_size,
  //             thr);
  // #endif
...
#ifdef ARIEL_DEBUG
  fprintf(stderr,
          "%u: Ariel mlm_malloc call allocates data at address: 0x%llx\n", thr,
          (uint64_t)real_ptr);
#endif
...
void ariel_mlm_free(void *ptr) {
...
#ifdef ARIEL_DEBUG
  fprintf(stderr,
          "Perform a mlm_free from Ariel (pointer = %p) on thread %lu\n", ptr,
          thr);
#endif
  ...
  PIN_GetLock(&mainLock, thr);
  for (ptr_list_itr = allocated_list.begin();
       ptr_list_itr != allocated_list.end(); ptr_list_itr++) {
    if (*ptr_list_itr == ptr) {
      found = true;
  ...
  if (found) {
#ifdef ARIEL_DEBUG
    fprintf(stderr,
            "ARIEL: Matched call to free, passing to Ariel free routine.\n");
#endif
    free(ptr);
    ...
    fprintf(stderr, "ARIEL: Call to free in Ariel did not find a matching "
                    "local allocation, this memory will be leaked.\n");
  }
}
...
void CUDARTAPI __cudaRegisterFunction(void **fatCubinHandle,
  ...
  ac.API.CA.register_function.fat_cubin_handle =
      (unsigned)(unsigned long long)fatCubinHandle;
...
__host__ cudaError_t CUDARTAPI cudaMemcpy(void *dst, const void *src,
  ...
  if (kind == cudaMemcpyDefault) {
    if (((size_t)src >= GLOBAL_HEAP_START) &&
        ((size_t)src < GLOBAL_HEAP_START + limit_size)) {
      if (((size_t)dst >= GLOBAL_HEAP_START) &&
          ((size_t)dst < GLOBAL_HEAP_START + limit_size)) {
        // device to device
  ...
  if (final_kind == cudaMemcpyHostToDevice) {
    if (count <= max_page_size) {
      // Single transfer (<=4k)
      bytes_copied =
          PIN_SafeCopy(&payload, static_cast<const uint8_t *>(src), count);
      ...
      while ((pages != 0) || (remainder != 0)) {
        if (pages != 0) {
          // Transfer 4k page
          bytes_copied = PIN_SafeCopy(
              &payload, static_cast<const uint8_t *>(src) + offset, 1 << 12);
          ...
          bytes_copied = PIN_SafeCopy(
              &payload, static_cast<const uint8_t *>(src) + offset, remainder);
...
__cudaRegisterVar(void **fatCubinHandle,
  ...
  ac.API.CA.register_var.fatCubinHandle =
      (unsigned)(unsigned long long)fatCubinHandle;
...
__host__ cudaError_t CUDARTAPI __maxActiveBlock(int *numBlocks,
                                                const char *hostFunc,
                                                int blockSize,
...
#ifdef ARIEL_DEBUG
  printf("CUDA fesimple return from "
         "cudaOccupancyMaxActiveBlocksPerMultiprocessorWithFlags with numBlock "
         "%d\n",
         gc.num_block);
  std::cout << gc.API_Return.name << std::endl;
#endif
...
VOID InstrumentRoutine(RTN rtn, VOID *args) {
  ...
  if (RTN_Name(rtn) == "ariel_enable" || RTN_Name(rtn) == "_ariel_enable" ||
      RTN_Name(rtn) == "__arielfort_MOD_ariel_enable") {
    fprintf(stderr, "Identified routine: ariel_enable, replacing with Ariel "
                    "equivalent...\n");
    RTN_Replace(rtn, (AFUNPTR)mapped_ariel_enable);
    fprintf(stderr, "Replacement complete.\n");
    if (StartupMode.Value() == 2) {
      fprintf(stderr, "Tool was called with auto-detect enable mode, setting "
                      "initial output to not be traced.\n");
      enable_output = false;
    }
    return;
  } else if (RTN_Name(rtn) == "gettimeofday" ||
             RTN_Name(rtn) == "_gettimeofday") {
    fprintf(stderr, "Identified routine: gettimeofday, replacing with Ariel "
                    "equivalent...\n");
    ...
    fprintf(stderr, "Identified routine: ariel_cycles, replacing with Ariel "
                    "equivalent..\n");
    RTN_Replace(rtn, (AFUNPTR)mapped_ariel_cycles);
    fprintf(stderr, "Replacement complete\n");
    return;
#if !defined(__APPLE__)
  } else if (RTN_Name(rtn) == "clock_gettime" ||
             RTN_Name(rtn) == "_clock_gettime" ||
             RTN_Name(rtn) == "__clock_gettime") {
    fprintf(stderr, "Identified routine: clock_gettime, replacing with Ariel "
                    "equivalent...\n");
    ...
    return;
             ...
             RTN_Name(rtn) == "mlm_malloc") {
    // This means we want a special malloc to be used (needs a TLB map inside
    // the virtual core)
    fprintf(
        stderr,
    ...
    return;
             ...
             RTN_Name(rtn) == "mlm_free") {
    fprintf(
        stderr,
        "Identified routine: mlm_free, replacing with Ariel equivalent...\n");
    RTN_Replace(rtn, (AFUNPTR)ariel_mlm_free);
    fprintf(stderr, "Replacement complete.\n");
    return;
  } else if ((InterceptMemAllocations.Value() > 0) &&
             RTN_Name(rtn) == "mlm_set_pool") {
    fprintf(stderr, "Identified routine: mlm_set_pool, replacing with Ariel "
                    "equivalent...\n");
    ...
    return;
             ...
             (RTN_Name(rtn) == "malloc" || RTN_Name(rtn) == "_malloc" ||
              ...
              RTN_Name(rtn) == "__libc_memalign" ||
              RTN_Name(rtn) == "_gfortran_malloc")) {

    fprintf(stderr, "Identified routine: malloc/_malloc, replacing with Ariel "
                    "equivalent...\n");
    RTN_Open(rtn);

    RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR)ariel_premalloc_instrument,
                   IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_INST_PTR, IARG_END);

    RTN_InsertCall(rtn, IPOINT_AFTER, (AFUNPTR)ariel_postmalloc_instrument,
                   IARG_FUNCRET_EXITPOINT_VALUE, IARG_END);

    RTN_Close(rtn);
  } else if ((InterceptMemAllocations.Value() > 0) &&
             (RTN_Name(rtn) == "free" || RTN_Name(rtn) == "_free" ||
              RTN_Name(rtn) == "__libc_free" ||
              RTN_Name(rtn) == "_gfortran_free")) {
    ...
    RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR)ariel_postfree_instrument,
                   IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);

    RTN_Close(rtn);
  } else if (RTN_Name(rtn) == "ariel_output_stats" ||
             RTN_Name(rtn) == "_ariel_output_stats" ||
             RTN_Name(rtn) == "__arielfort_MOD_ariel_output_stats") {
    fprintf(stderr, "Identified routine: ariel_output_stats, replacing with "
                    "Ariel equivalent..\n");
    RTN_Replace(rtn, (AFUNPTR)mapped_ariel_output_stats);
    fprintf(stderr, "Replacement complete\n");
    return;
  } else if (RTN_Name(rtn) == "ariel_output_stats_buoy" ||
             RTN_Name(rtn) == "_ariel_output_stats_buoy") {
    fprintf(stderr, "Identified routine: ariel_output_stats_buoy, replacing "
                    "with Ariel equivalent..\n");
    RTN_Replace(rtn, (AFUNPTR)mapped_ariel_output_stats_buoy);
    fprintf(stderr, "Replacement complete\n");
    return;
  } else if (RTN_Name(rtn) == "ariel_flushline" ||
             RTN_Name(rtn) == "_ariel_flushline") {
    fprintf(stderr, "Identified routine: ariel_flushline, replacing with Ariel "
                    "equivalent..\n");
    RTN_Replace(rtn, (AFUNPTR)mapped_ariel_flushline);
    fprintf(stderr, "Replacement complete\n");
    return;
  } else if (RTN_Name(rtn) == "ariel_fence" ||
             RTN_Name(rtn) == "_ariel_fence") {
    fprintf(
        stderr,
        "Identified routine: ariel_fence, replacing with Ariel equivalent..\n");
    RTN_Replace(rtn, (AFUNPTR)mapped_ariel_fence);
    fprintf(stderr, "Replacement complete\n");
    return;
  } else if (RTN_Name(rtn) == "ariel_mmap_mlm" ||
             RTN_Name(rtn) == "_ariel_mmap_mlm") {
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)ariel_mmap_mlm);
    return;
  } else if (RTN_Name(rtn) == "ariel_munmap_mlm" ||
             RTN_Name(rtn) == "_ariel_munmap_mlm") {
    return;
#ifdef HAVE_CUDA
  } else if (RTN_Name(rtn) == "__cudaRegisterFatBinary") {
    fprintf(stderr, "Identified routine: __cudaRegisterFatBinary, replacing "
                    "with GPGPU-Sim equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)__cudaRegisterFatBinary);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "__cudaRegisterFunction") {
    fprintf(stderr, "Identified routine: __cudaRegisterFunction, replacing "
                    "with GPGPU-Sim equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)__cudaRegisterFunction);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "cudaMalloc") {
    fprintf(stderr, "Identified routine: cudaMalloc, replacing with GPGPU-Sim "
                    "equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)cudaMalloc);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "cudaMemcpy") {
    fprintf(stderr, "Identified routine: cudaMemcpy, replacing with GPGPU-Sim "
                    "equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)cudaMemcpy);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "cudaConfigureCall") {
    fprintf(stderr, "Identified routine: cudaConfigureCall, replacing with "
                    "GPGPU-Sim equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)cudaConfigureCall);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "cudaSetupArgument") {
    fprintf(stderr, "Identified routine: cudaSetupArgument, replacing with "
                    "GPGPU-Sim equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)cudaSetupArgument);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "cudaLaunch") {
    fprintf(stderr, "Identified routine: cudaLaunch, replacing with GPGPU-Sim "
                    "equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)cudaLaunch);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "cudaFree") {
    fprintf(stderr, "Identified routine: cudaFree, replacing with GPGPU-Sim "
                    "equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)cudaFree);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "cudaGetLastError") {
    fprintf(stderr, "Identified routine: cudaGetLastError, replacing with "
                    "GPGPU-Sim equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)cudaGetLastError);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) == "__cudaRegisterVar") {
    fprintf(stderr, "Identified routine: __cudaRegisterVar, replacing with "
                    "GPGPU-Sim equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)__cudaRegisterVar);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
  } else if (RTN_Name(rtn) ==
             "cudaOccupancyMaxActiveBlocksPerMultiprocessorWithFlags") {
    fprintf(stderr, "Identified routine: "
                    "cudaOccupancyMaxActiveBlocksPerMultiprocessorWithFlags, "
                    "replacing with GPGPU-Sim equivilant...\n");
    AFUNPTR ret = RTN_Replace(rtn, (AFUNPTR)__maxActiveBlock);
    fprintf(stderr, "Replacement complete. (%p)\n", ret);
    return;
#endif
  } else if (UseMallocMap.Value() != "") {
    if (RTN_Name(rtn) == "ariel_malloc_flag" ||
        RTN_Name(rtn) == "_ariel_malloc_flag") {

      fprintf(stderr, "Identified routine: ariel_malloc_flag, replacing with "
                      "Ariel equivalent..\n");
...
int main(int argc, char *argv[]) {
  ...
  if (funcProfileLevel > 0) {
    std::cout << "SSTARIEL: Function profile level is configured to: "
              << funcProfileLevel << std::endl;
  } else {
  ...
  if (NULL != override_pool_name) {
    ...
    fprintf(stderr,
            "ARIEL-SST: Use pool: %" PRIu32
            " instead of application provided\n",
            overridePool);
  } else {
    fprintf(stderr, "ARIEL-SST: Did not find ARIEL_OVERRIDE_POOL in the "
                    "environment, no override applies.\n");
  }
  ...
  if (writeTrace) {
    if (SSTVerbosity.Value() > 0) {
      printf("SSTARIEL: Performing write tracing (this is an expensive "
             "operation.)\n");
    }
  }
  ...
  tunnelmgr =
      new SST::Core::Interprocess::SHMChild<ArielTunnel>(SSTNamedPipe.Value());
  tunnel = tunnelmgr->getTunnel();
#ifdef HAVE_CUDA
  tunnelRmgr = new SST::Core::Interprocess::SHMChild<GpuReturnTunnel>(
      SSTNamedPipe2.Value());
  tunnelDmgr = new SST::Core::Interprocess::SHMChild<GpuDataTunnel>(
      SSTNamedPipe3.Value());
  ...
  if (KeepMallocStackTrace.Value() == 1) {
    ...
    instPtrsList.resize(core_count); // Need core_count sets of instruction
                                     // pointers (to avoid locks)
  }
  ...
  for (int i = 0; i < core_count; i++) {
    ...
    if (KeepMallocStackTrace.Value() == 1) {
      ...
      if (btfiles.back() == NULL) {
        fprintf(
            stderr,
            "ARIEL ERROR: could not create and/or open backtrace file: %s\n",
            filename.c_str());
  ...
  if (StartupMode.Value() == 1) {
    ...
    fprintf(stderr, "ARIEL: Tool is configured to suspend profiling until "
                    "program control\n");
    enable_output = false;
  } else if (StartupMode.Value() == 2) {
    fprintf(stderr,
            "ARIEL: Tool is configured to attempt auto detect of profiling\n");
    fprintf(stderr, "ARIEL: Initial mode will be to enable profiling unless "
                    "ariel_enable function is located\n");
    enable_output = true;
  }

  /* If not using ariel_enable, then gettimeofday/clock_gettime always return
   * simulated time */
  offset_tv.tv_sec = 0;
  offset_tv.tv_usec = 0;
#if !defined(__APPLE__)
