...
KNOB <UINT32> PerformWriteTrace(KNOB_MODE_WRITEONCE, "pintool",
                                "w", "0",
                                "Perform write tracing (i.e copy values directly into SST memory operations) (0 = disabled, 1 = enabled)");
...
KNOB <UINT32> StartupMode(KNOB_MODE_WRITEONCE, "pintool",
                          "s", "1",
                          "Mode for configuring profile behavior, 1 = start enabled, 0 = start disabled, 2 = attempt auto detect");
KNOB <UINT32> InterceptMemAllocations(KNOB_MODE_WRITEONCE, "pintool",
                                      "m", "1",
                                      "Should intercept multi-level memory allocations, mallocs, and frees, 1 = start enabled, 0 = start disabled");
KNOB <string> UseMallocMap(KNOB_MODE_WRITEONCE, "pintool",
                           "u", "",
                           "Should intercept ariel_malloc_flag() and interpret using a malloc map: specify filename or leave blank for disabled");
KNOB <UINT32> KeepMallocStackTrace(KNOB_MODE_WRITEONCE, "pintool",
                                   "k", "1",
                                   "Should keep shadow stack and dump on malloc calls. 1 = enabled, 0 = disabled");
...
VOID ariel_print_stack(UINT32 thr, UINT64 allocSize, UINT64 allocAddr, UINT64 allocIndex) {
    ...
    for (vector<StackRecord>::reverse_iterator rit = arielStack[thr].rbegin();
        ...
        BT_PRINTF("0x%"
                      ...
                      ",0x%"
                      PRIx64
                      ",%s", rit->getTarget(), rit->getInstPtr(), ((depth == 0) ? "\n" : ""));
        depth--;
    }
...
void mapped_ariel_enable() {
    ...
    fprintf(stderr,
            "ARIEL: Enabling memory and instruction tracing from program control at simulated Ariel cycle %"
    PRIu64
    ".\n",
        tunnel->getCycles());
    fflush(stdout);
...
void ariel_mlm_free(void *ptr) {
    ...
    if (found) {
        ...
        fprintf(stderr,
                "ARIEL: Call to free in Ariel did not find a matching local allocation, this memory will be leaked.\n");
    }
}
...
VOID InstrumentRoutine(RTN rtn, VOID *args) {
    ...
    if (RTN_Name(rtn) == "ariel_enable" || RTN_Name(rtn) == "_ariel_enable" ||
        ...
        if (StartupMode.Value() == 2) {
            fprintf(stderr,
                    "Tool was called with auto-detect enable mode, setting initial output to not be traced.\n");
            enable_output = false;
        }
        return;
        ...
        RTN_Name(rtn) == "__libc_malloc" || RTN_Name(rtn) == "__libc_memalign" ||
        ...
        fprintf(stderr,
                "Identified routine: ariel_output_stats, replacing with Ariel equivalent..\n");
        ...
        fprintf(stderr,
                "Identified routine: ariel_output_stats_buoy, replacing with Ariel equivalent..\n");
        ...
        if (RTN_Name(rtn) == "ariel_malloc_flag" || RTN_Name(rtn) == "_ariel_malloc_flag") {
            ...
            fprintf(stderr,
                    "Identified routine: ariel_malloc_flag, replacing with Ariel equivalent..\n");
...
int main(int argc, char *argv[]) {
    ...
    if (funcProfileLevel > 0) {
        std::cout << "SSTARIEL: Function profile level is configured to: " << funcProfileLevel
                  << std::endl;
    } else {
    ...
    if (NULL != override_pool_name) {
        ...
        fprintf(stderr, "ARIEL-SST: Use pool: %"
        PRIu32
    ...
    if (KeepMallocStackTrace.Value() == 1) {
        ...
        instPtrsList.resize(
            core_count);    // Need core_count sets of instruction pointers (to avoid locks)
    }
    ...
    fprintf(stderr, "ARIEL-SST PIN tool activating with %"
    PRIu32
    " threads\n", core_count);
    fflush(stdout);
    ...
    fprintf(stderr, "ARIEL: Default memory pool set to %"
    PRIu32
    "\n", default_pool);
    ...
    if (StartupMode.Value() == 1) {
        ...
        fprintf(stderr,
                "ARIEL: Initial mode will be to enable profiling unless ariel_enable function is located\n");
        enable_output = true;
    }
