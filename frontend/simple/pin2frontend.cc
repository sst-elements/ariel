...
Pin2Frontend::Pin2Frontend(ComponentId_t id, Params &params, uint32_t cores,
                           uint32_t maxCoreQueueLen, uint32_t defMemPool)
    : ArielFrontend(id, params, cores, maxCoreQueueLen, defMemPool) {

  int verbosity = params.find<int>("verbose", 0);
  output = new SST::Output("Pin2Frontend[@f:@l:@p] ", verbosity, 0,
                           SST::Output::STDOUT);
  ...
  if ("" == ariel_tool) {
    output->fatal(CALL_INFO, -1,
                  "The arieltool parameter specifying which PIN tool to run "
                  "was not specified\n");
  }
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Model specifies that there are %" PRIu32
                  " application arguments\n",
                  app_argc);

  uint32_t pin_startup_mode = (uint32_t)params.find<uint32_t>("arielmode", 2);
  uint32_t intercept_mem_allocations =
      (uint32_t)params.find<uint32_t>("arielinterceptcalls", 0);

  switch (intercept_mem_allocations) {
  case 0:
    output->verbose(CALL_INFO, 1, 0,
                    "Interception and instrumentation of multi-level memory "
                    "and malloc/free calls is DISABLED.\n");
    break;
  default:
    output->verbose(CALL_INFO, 1, 0,
                    "Interception and instrumentation of multi-level memory "
                    "and malloc/free calls is ENABLED.\n");
    break;
  }

  uint32_t keep_malloc_stack_trace =
      (uint32_t)params.find<uint32_t>("arielstack", 0);
  output->verbose(CALL_INFO, 1, 0,
                  "Tracking the stack and dumping on malloc calls is %s.\n",
                  keep_malloc_stack_trace == 1 ? "ENABLED" : "DISABLED");

  std::string malloc_map_filename =
      params.find<std::string>("mallocmapfile", "");
  if (malloc_map_filename == "") {
    output->verbose(CALL_INFO, 1, 0, "Malloc map file is DISABLED\n");
  } else {
    output->verbose(CALL_INFO, 1, 0,
                    "Malloc map file is ENABLED, using file '%s'\n",
                    malloc_map_filename.c_str());
  }

  tunnelmgr = new SST::Core::Interprocess::SHMParent<ArielTunnel>(
      id, core_count, maxCoreQueueLen);
...
#ifdef HAVE_CUDA
  tunnelRmgr = new SST::Core::Interprocess::SHMParent<GpuReturnTunnel>(
      id, core_count, maxCoreQueueLen);
  tunnelDmgr = new SST::Core::Interprocess::SHMParent<GpuDataTunnel>(
      id, core_count, maxCoreQueueLen);
  ...
  const uint32_t launch_param_count =
      (uint32_t)params.find<uint32_t>("launchparamcount", 0);
  const uint32_t pin_arg_count = 37 + launch_param_count;

  execute_args = (char **)malloc(sizeof(char *) * (pin_arg_count + app_argc));

  const uint32_t profileFunctions =
      (uint32_t)params.find<uint32_t>("profilefunctions", 0);
  ...
  if (malloc_map_filename != "") {
    ...
    execute_args[arg++] =
        (char *)malloc(sizeof(char) * (malloc_map_filename.size() + 1));
  ...
  for (uint32_t aa = 0; aa < app_argc; ++aa) {
    ...
    output->verbose(CALL_INFO, 1, 0,
                    "Found application argument %" PRIu32 " (%s) = %s\n", aa,
                    argv_buffer, argv_i.c_str());
  ...
  if (pin_env_count > -1) {
    ...
    for (int32_t next_env_param = 0; next_env_param < pin_env_count;
         next_env_param++) {
      ...
      if ("" == env_name) {
        output->fatal(CALL_INFO, -1,
                      "Parameter: %s environment variable name is empty",
                      env_name_buffer);
      }
      ...
      execute_env.insert(
          std::pair<std::string, std::string>(env_name, env_value));
    }
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Completed processing application arguments.\n");
...
ArielTunnel *Pin2Frontend::getTunnel() { return tunnel; }

#ifdef HAVE_CUDA
GpuReturnTunnel *Pin2Frontend::getReturnTunnel() { return tunnelR; }

GpuDataTunnel *Pin2Frontend::getDataTunnel() { return tunnelD; }
#endif

int Pin2Frontend::forkPINChild(const char *app, char **args,
  ...
  while (NULL != args[next_arg_index]) {
    ...
    while ('\0' != args[next_arg_index][copy_char_index]) {
      full_execute_line[next_line_index++] =
          args[next_arg_index][copy_char_index++];
    }
  ...
  if (the_child != 0) {
    ...
    if (check > 0) {
      ...
      if (WIFEXITED(pstat) == true) {
        ...
        output->fatal(CALL_INFO, 1,
                      "Launching trace child failed!  Child Terminated With "
                      "Signal %d; Core Dump File Created = %d\n",
      ...
      perror("waitpid");
      output->fatal(CALL_INFO, 1,
                    "Waitpid returned an error, errno = %d.  Did the child "
                    "ever even start?\n",
                    errno);
    }
    ...
    if (0 == app_env.size()) {
      ...
      char **execute_env_cp =
          (char **)malloc(sizeof(char *) * (app_env.size() + 1));
      uint32_t next_env_cp_index = 0;

      for (auto env_itr = app_env.begin(); env_itr != app_env.end();
           env_itr++) {
        char *execute_env_nv_pair =
            (char *)malloc(sizeof(char) * (2 + env_itr->first.size() +
                                           env_itr->second.size()));
