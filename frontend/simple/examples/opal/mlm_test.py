...
for next_ring_stop in range((cores_per_group + memory_controllers_per_group + l3cache_blocks_per_group) * groups):
    ...
    if next_ring_stop == 0:
        ...
        rtr_link_negative.connect((router_map["rtr.0"], "port1", ring_latency), (router_map["rtr." + str(
            ((cores_per_group + memory_controllers_per_group + l3cache_blocks_per_group) * groups) - 1)], "port0",
                                                                                 ring_latency))
    elif next_ring_stop == ((cores_per_group + memory_controllers_per_group + l3cache_blocks_per_group) * groups) - 1:
        rtr_link_positive = sst.Link("rtr_pos_" + str(next_ring_stop))
        rtr_link_positive.connect((router_map["rtr." + str(next_ring_stop)], "port0", ring_latency),
                                  (router_map["rtr.0"], "port1", ring_latency))
        rtr_link_negative = sst.Link("rtr_neg_" + str(next_ring_stop))
        rtr_link_negative.connect((router_map["rtr." + str(next_ring_stop)], "port1", ring_latency),
                                  (router_map["rtr." + str(next_ring_stop - 1)], "port0", ring_latency))
    else:
        rtr_link_positive = sst.Link("rtr_pos_" + str(next_ring_stop))
        rtr_link_positive.connect((router_map["rtr." + str(next_ring_stop)], "port0", ring_latency),
                                  (router_map["rtr." + str(next_ring_stop + 1)], "port1", ring_latency))
        rtr_link_negative = sst.Link("rtr_neg_" + str(next_ring_stop))
        rtr_link_negative.connect((router_map["rtr." + str(next_ring_stop)], "port1", ring_latency),
                                  (router_map["rtr." + str(next_ring_stop - 1)], "port0", ring_latency))
...
for next_group in range(groups):
    ...
    for next_active_core in range(cores_per_group):
        for next_l3_cache_block in range(l3_cache_per_core):
            ...
            l3_ring_link.connect((l3cache, "directory", ring_latency),
        ...
        if next_core_id < cores_per_group * groups / 2:
            arielMMULink.connect((ariel, "cache_link_%d" % next_core_id, ring_latency),
                                 (mmu, "cpu_to_mmu%d" % next_core_id, ring_latency))
            ArielOpalLink.connect((ariel, "opal_link_%d" % next_core_id, ring_latency),
                                  (opal, "requestLink%d" % (next_core_id + cores_per_group * groups / 2), ring_latency))
            MMUCacheLink.connect((mmu, "mmu_to_cache%d" % next_core_id, ring_latency),
                                 (l1, "high_network_0", ring_latency))
            PTWOpalLink.connect((mmu, "ptw_to_opal%d" % next_core_id, "50ps"),
                                (opal, "requestLink%d" % next_core_id, "50ps"))
            arielMMULink.setNoCut()
            PTWOpalLink.setNoCut()
            MMUCacheLink.setNoCut()
        else:
            PTWMemLink.connect((mmu, "ptw_to_mem%d" % (next_core_id - cores_per_group * groups / 2), ring_latency),
                               (l1, "high_network_0", ring_latency))
        # '''
        ...
        l2_ring_link.connect((l2, "cache", ring_latency),
                             (router_map["rtr." + str(next_network_id)], "port2", ring_latency))
    ...
    for next_mem_ctrl in range(memory_controllers_per_group):
        ...
        netLink.connect((dc, "network", ring_latency),
                        (router_map["rtr." + str(next_network_id)], "port2", ring_latency))
