...
Pin3Frontend::Pin3Frontend(ComponentId_t id, Params &params, uint32_t cores,
  ...
  output = new SST::Output("Pin3Frontend[@f:@l:@p] ", verbosity, 0,
                           SST::Output::STDOUT);
  ...
  tunnelmgr = new SST::Core::Interprocess::MMAPParent<ArielTunnel>(
...
#ifdef HAVE_CUDA
  tunnelRmgr = new SST::Core::Interprocess::MMAPParent<GpuReturnTunnel>(
      id, core_count, maxCoreQueueLen);
  tunnelDmgr = new SST::Core::Interprocess::MMAPParent<GpuDataTunnel>(
      id, core_count, maxCoreQueueLen);
...
ArielTunnel *Pin3Frontend::getTunnel() { return tunnel; }

#ifdef HAVE_CUDA
GpuReturnTunnel *Pin3Frontend::getReturnTunnel() { return tunnelR; }

GpuDataTunnel *Pin3Frontend::getDataTunnel() { return tunnelD; }
#endif

int Pin3Frontend::forkPINChild(const char *app, char **args,
