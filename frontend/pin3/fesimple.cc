...
VOID WriteInstructionWrite(ADDRINT *address, UINT32 writeSize, THREADID thr,
  ...
  if (writeTrace) {
    ...
    PIN_SafeCopy(&ac.inst.payload[0], address,
                 ARIEL_MIN(writeSize, (UINT32)ARIEL_MAX_PAYLOAD_SIZE));
  }
  /*
...
#ifdef HAVE_CUDA
  tunnelRmgr = new SST::Core::Interprocess::MMAPChild_Pin3<GpuReturnTunnel>(
      SSTNamedPipe2.Value());
  tunnelDmgr = new SST::Core::Interprocess::MMAPChild_Pin3<GpuDataTunnel>(
      SSTNamedPipe3.Value());
