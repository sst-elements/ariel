...
ArielMemoryManagerSimple::ArielMemoryManagerSimple(ComponentId_t id,
                                                   Params &params)
...
ArielMemoryManagerSimple::~ArielMemoryManagerSimple() = default;

void ArielMemoryManagerSimple::allocate(const uint64_t size,
  ...
  // We will do all of our allocated based on whole pages, inefficient maybe but
  ...
  for (uint64_t bytesLeft = 0; bytesLeft < roundedSize; bytesLeft += pageSize) {
    if (freePages.empty()) {
      output->fatal(CALL_INFO, -1,
                    "Requested a memory allocation of size: %" PRIu64
...
uint64_t ArielMemoryManagerSimple::translateAddress(uint64_t virtAddr) {
  ...
  if (page_itr != pageTable.end()) {
    ...
    output->verbose(CALL_INFO, 4, 0,
                    "Page table hit: virtual address=%" PRIu64
                    " hit, virtual page start=%" PRIu64 ", virtual end=%" PRIu64
                    ", translates to phys page start=%" PRIu64
                    " translates to: phys address: %" PRIu64
                    " (offset added to phys start=%" PRIu64 ")\n",
                    virtAddr, page_itr->first, page_itr->first + pageSize,
