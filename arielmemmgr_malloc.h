...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      ArielMemoryManagerMalloc, "ariel", "MemoryManagerMalloc",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "MLM memory manager which supports malloc/free in different memory pools",
      SST::ArielComponent::ArielMemoryManager)

#define ARIEL_MEMMGR_MALLOC_ELI_PARAMS                                         \
  ARIEL_ELI_MEMMGR_CACHE_PARAMS,                                               \
      ...
      {"pagecount%(memorylevels)d", "Page count for memory Level x",           \
       "131072"},                                                              \
  {                                                                            \
    "page_populate_%(memorylevels)d",                                          \
        "Pre-populate/partially pre-populate a page table for a level in "     \
        "memory, this is the file to read in.",                                \
        ""                                                                     \
  }
#define ARIEL_MEMMGR_MALLOC_ELI_STATS                                          \
  ARIEL_ELI_MEMMGR_CACHE_STATS,                                                \
      {"bytes_allocated_in_pool",                                              \
       "Number of bytes allocated explicitly to memory pool <SubId>. Count "   \
       "is # of allocations",                                                  \
       "bytes", 3},                                                            \
      {"bytes_freed_from_pool",                                                \
       "Number of bytes freed explicitly from memory pool <SubId>. Count is "  \
       "# of frees",                                                           \
       "bytes", 3},                                                            \
      {"demand_page_allocs",                                                   \
       "Number of on-demand page allocations in memory pool <SubId>", "count", \
       3}
...
private:
  ...
  struct mallocInfo {
    ...
    mallocInfo(uint64_t size, uint32_t level,
               std::unordered_set<uint64_t> *VAKeys)
        : size(size), level(level), VAKeys(VAKeys){};
  };

  std::map<uint64_t, uint64_t>
      mallocPrimaryVAMap; // Map VA of each PA to the primary VA of the malloc
                          // -> used to find the mallocInfo
  std::map<uint64_t, uint64_t>
      mallocTranslations; // Map VA to PA for mallocs -> primary lookup
  std::map<uint64_t, mallocInfo>
      mallocInformation; // Map mallocID to information about the malloc -> use
                         // for frees/allocs
