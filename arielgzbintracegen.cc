...
ArielCompressedBinaryTraceGenerator::ArielCompressedBinaryTraceGenerator(
  ...
  buffer = (char *)malloc(sizeof(uint64_t) + sizeof(uint64_t) + sizeof(char) +
                          sizeof(uint32_t));
}
...
void ArielCompressedBinaryTraceGenerator::publishEntry(
    const uint64_t picoS, const uint64_t physAddr, const uint32_t reqLength,
  ...
  copy(&buffer[sizeof(uint64_t) + sizeof(char) + sizeof(uint64_t)], &reqLength,
       sizeof(uint32_t));

  gzwrite(traceFile, buffer,
...
void ArielCompressedBinaryTraceGenerator::copy(char *dest, const void *src,
                                               const size_t length) {
