...
#define ARIEL_CORE_VERBOSE(LEVEL, OUTPUT)                                      \
  if (verbosity >= (LEVEL))                                                    \
  OUTPUT
...
ArielCore::ArielCore(ComponentId_t id, ArielTunnel *tunnel,
...
#endif
                     uint32_t thisCoreID, uint32_t maxPendTrans, Output *out,
                     uint32_t maxIssuePerCyc, uint32_t maxQLen,
                     uint64_t cacheLineSz, ArielMemoryManager *memMgr,
                     const uint32_t perform_address_checks, Params &params)
  ...
  output->verbose(CALL_INFO, 2, 0,
                  "Creating core with ID %" PRIu32
                  ", maximum queue length=%" PRIu32 ", max issue is: %" PRIu32
                  "\n",
  ...
  pendingTransactions =
      new std::unordered_map<SimpleMem::Request::id_t, SimpleMem::Request *>();
  ...
  statReadRequestSizes =
      registerStatistic<uint64_t>("read_request_sizes", subID);
  statWriteRequestSizes =
      registerStatistic<uint64_t>("write_request_sizes", subID);
  statSplitReadRequests =
      registerStatistic<uint64_t>("split_read_requests", subID);
  statSplitWriteRequests =
      registerStatistic<uint64_t>("split_write_requests", subID);
  ...
  statInstructionCount =
      registerStatistic<uint64_t>("instruction_count", subID);
  ...
  memmgr->registerInterruptHandler(
      coreID, new ArielMemoryManager::InterruptHandler<ArielCore>(
                  this, &ArielCore::handleInterrupt));
  ...
  if (enableTracing) {
    ...
    traceGen = dynamic_cast<ArielTraceGenerator *>(
        loadModule(traceGenName, interfaceParams));
...
void ArielCore::setCacheLink(SimpleMem *newLink) { cacheLink = newLink; }

#ifdef HAVE_CUDA
void ArielCore::setGpuLink(Link *gpulink) { GpuLink = gpulink; }
#endif

void ArielCore::printTraceEntry(const bool isRead, const uint64_t address,
...
void ArielCore::commitReadEvent(const uint64_t address,
                                const uint64_t virtAddress,
                                const uint32_t length) {
  if (length > 0) {
    auto *req =
        new SimpleMem::Request(SimpleMem::Request::Read, address, length);
    req->setVirtualAddress(virtAddress);
#ifdef HAVE_CUDA
    if (isGpuEx()) {
      pending_transaction_count++;
      pendingGpuTransactions->insert(
          std::pair<SimpleMem::Request::id_t, SimpleMem::Request *>(req->id,
                                                                    req));
    } else {
#endif
      pending_transaction_count++;
...
#endif
    if (enableTracing) {
      printTraceEntry(true, (const uint64_t)req->addrs[0],
                      (const uint32_t)length);
    }
...
void ArielCore::commitWriteEvent(const uint64_t address,
  ...
  if (length > 0) {
    auto *req =
        new SimpleMem::Request(SimpleMem::Request::Write, address, length);
    req->setVirtualAddress(virtAddress);
...
#endif
    if (enableTracing) {
      printTraceEntry(false, (const uint64_t)req->addrs[0],
                      (const uint32_t)length);
    }
...
// Although flushes are not technically memory transaction events like writing
// and reading, they are treated as such for the purposes of determining the
// number of pending transactions in the queue. Thus, this function is referred
// to as a commit
...
void ArielCore::commitFlushEvent(const uint64_t address,
  ...
  if (length > 0) {
    /*  Todo: should the request specify the physical address, or the virtual
     * address? */
    auto *req = new SimpleMem::Request(SimpleMem::Request::FlushLineInv,
                                       address, length);
...
void ArielCore::handleEvent(SimpleMem::Request *event) {
  ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0,
                                        "Core %" PRIu32
                                        " handling a memory event.\n",
                                        coreID));
  ...
  if (pendingGpuTransactions->find(mev_id) != pendingGpuTransactions->end()) {
    ...
    output->verbose(CALL_INFO, 16, 0,
                    "CUDA: Total ACK %" PRIu32 ", Page ACK %" PRIu32 "\n",
    ...
    if (getKind() == cudaMemcpyHostToDevice) {
      if ((getPageAckTransfer() == getPageTransfer())) {
        if (getAckTransfer() == getTotalTransfer()) {
          ...
          output->verbose(CALL_INFO, 16, 0,
                          "CUDA: Moving to GPU stage %" PRIu32 "\n",
                          getBaseAddress());
          ...
          output->verbose(CALL_INFO, 16, 0,
                          "CUDA: New page data with total size %" PRIu32 ":",
                          current_transfer);
          if (verbosity >= 16) {
            for (int i = 0; i < current_transfer; i++)
              output->verbose(CALL_INFO, 16, 0, "%" PRIu32 ", ", data[i]);
            output->verbose(CALL_INFO, 16, 0, "\n");
          }

          // Updated Page related variables, remove from tunnel, point to new
          // data
      ...
      if (getRemainingPageTransfer() != 0) {
        ...
        while ((getOpenTransactions() > 0) &&
               (getRemainingPageTransfer() > 0)) {
          index = getCurrentAddress() - getBaseAddress();
          current_transfer = (getRemainingPageTransfer() > 64)
                                 ? 64
                                 : getRemainingPageTransfer();

          output->verbose(CALL_INFO, 16, 0,
                          "CUDA: Transfer of  %" PRIu32 " to index %" PRIu64,
          ...
          ArielWriteEvent *awe;
          awe = new ArielWriteEvent(getCurrentAddress(), current_transfer,
                                    &getDataAddress()[index]);
          handleWriteRequest(awe);
          setCurrentAddress(getCurrentAddress() + current_transfer);
          setRemainingPageTransfer(getRemainingPageTransfer() -
                                   current_transfer);
        }
      } else {
      ...
      if ((getAckTransfer() == getTotalTransfer())) {
        ...
        output->verbose(CALL_INFO, 16, 0,
                        "CUDA: Data returned to fesimple of size %" PRIu32 ":",
                        getTotalTransfer());
        if (verbosity >= 16) {
        ...
        while ((getOpenTransactions() > 0) && (getRemainingTransfer() > 0)) {
          if (getRemainingTransfer() <= 64) {
            are =
                new ArielReadEvent(getCurrentAddress(), getRemainingTransfer());
  ...
  if (find_entry != pendingTransactions->end()) {
    ...
    ARIEL_CORE_VERBOSE(
        ...
        output->verbose(CALL_INFO, 4, 0,
                        "Correctly identified event in pending transactions, "
                        "removing from list, before there are: %" PRIu32
    ...
    output->fatal(CALL_INFO, -4,
                  "Memory event response to core: %" PRIu32
...
bool ArielCore::handleInterrupt(ArielMemoryManager::InterruptAction action) {
  ...
  ARIEL_CORE_VERBOSE(
      4, output->verbose(CALL_INFO, 4, 0,
                         "Core %" PRIu32 " received an interrupt.\n", coreID));
  ...
  default:
    output->fatal(CALL_INFO, -4,
                  "Received an unknown interrupt on core %" PRIu32 "\n",
                  coreID);
  }
...
void ArielCore::halt() { isHalted = true; }

void ArielCore::stall() { isStalled = true; }

#ifdef HAVE_CUDA
void ArielCore::setKind(cudaMemcpyKind memcpyKind) { kind = memcpyKind; }

void ArielCore::gpu() { isGpu = true; }
...
void ArielCore::setPhysicalAddresses(SST::Event *ev) {
  ...
  current_transfer =
      (getRemainingTransfer() > 64) ? 64 : getRemainingTransfer();
  ...
  if ((addr_offset + current_transfer <= cacheLineSize)) {
    ...
    uint64_t rightAddr =
        (getCurrentAddress() + ((uint64_t)cacheLineSize)) - addr_offset;
...
void ArielCore::setMidTransfer(bool midTx) { midTransfer = midTx; }

void ArielCore::setRemainingPageTransfer(size_t tx) {
  remainingPageTransfer = tx;
}

void ArielCore::setPageTransfer(size_t tx) { pageTransfer = tx; }

void ArielCore::setTotalTransfer(size_t tx) { totalTransfer = tx; }

void ArielCore::setPageAckTransfer(size_t tx) { pageAckTransfer = tx; }

void ArielCore::setAckTransfer(size_t tx) { ackTransfer = tx; }

void ArielCore::setRemainingTransfer(size_t tx) { remainingTransfer = tx; }
...
void ArielCore::fence() {
  ARIEL_CORE_VERBOSE(
      4, output->verbose(CALL_INFO, 4, 0,
                         "Core: %" PRIu32
                         " FENCE:  Current pending transaction count: %" PRIu32
                         " (%" PRIu32 ")\n",
                         coreID, pending_transaction_count,
...
void ArielCore::unfence() {
  ARIEL_CORE_VERBOSE(
      4, output->verbose(
             CALL_INFO, 4, 0,
             "Core: %" PRIu32
             " UNFENCE:  Current pending transaction count: %" PRIu32
             " (%" PRIu32 ")\n",
...
void ArielCore::handleSwitchPoolEvent(ArielSwitchPoolEvent *aSPE) {
  ARIEL_CORE_VERBOSE(2, output->verbose(CALL_INFO, 2, 0,
                                        "Core: %" PRIu32
                                        " set default memory pool to: %" PRIu32
                                        "\n",
                                        coreID, aSPE->getPool()));
...
void ArielCore::createSwitchPoolEvent(uint32_t newPool) {
  auto *ev = new ArielSwitchPoolEvent(newPool);
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(
      4, output->verbose(CALL_INFO, 4, 0,
                         "Generated a switch pool event on core %" PRIu32
                         ", new level is: %" PRIu32 "\n",
                         coreID, newPool));
}

void ArielCore::createNoOpEvent() {
  auto *ev = new ArielNoOpEvent();
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(
      4,
      output->verbose(CALL_INFO, 4, 0,
                      "Generated a No Op event on core %" PRIu32 "\n", coreID));
}

void ArielCore::createReadEvent(uint64_t address, uint32_t length) {
  auto *ev = new ArielReadEvent(address, length);
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0,
                                        "Generated a READ event, addr=%" PRIu64
                                        ", length=%" PRIu32 "\n",
                                        address, length));
}

void ArielCore::createAllocateEvent(uint64_t vAddr, uint64_t length,
                                    uint32_t level, uint64_t instPtr) {
  auto *ev = new ArielAllocateEvent(vAddr, length, level, instPtr);
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(
      2, output->verbose(CALL_INFO, 2, 0,
                         "Generated an allocate event, vAddr(map)=%" PRIu64
                         ", length=%" PRIu64 " in level %" PRIu32
                         " from IP %" PRIx64 "\n",
                         vAddr, length, level, instPtr));
}

void ArielCore::createMmapEvent(uint32_t fileID, uint64_t vAddr,
                                uint64_t length, uint32_t level,
                                uint64_t instPtr) {
  auto *ev = new ArielMmapEvent(fileID, vAddr, length, level, instPtr);
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(
      2, output->verbose(CALL_INFO, 2, 0,
                         "Generated an mmap event, vAddr(map)=%" PRIu64
                         ", length=%" PRIu64 " in level %" PRIu32
                         " from IP %" PRIx64 "\n",
                         vAddr, length, level, instPtr));
}
...
void ArielCore::createFreeEvent(uint64_t vAddr) {
  auto *ev = new ArielFreeEvent(vAddr);
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(
      2, output->verbose(CALL_INFO, 2, 0,
                         "Generated a free event for virtual address=%" PRIu64
                         "\n",
                         vAddr));
}
...
void ArielCore::createWriteEvent(uint64_t address, uint32_t length,
                                 const uint8_t *payload) {
  auto *ev = new ArielWriteEvent(address, length, payload);
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0,
                                        "Generated a WRITE event, addr=%" PRIu64
                                        ", length=%" PRIu32 "\n",
                                        address, length));
}

void ArielCore::createFlushEvent(uint64_t vAddr) {
  auto *ev = new ArielFlushEvent(vAddr, cacheLineSize);
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(
      4, output->verbose(CALL_INFO, 4, 0, "Generated a FLUSH event.\n"));
}

void ArielCore::createFenceEvent() {
  auto *ev = new ArielFenceEvent();
  coreQ->push(ev);

  ARIEL_CORE_VERBOSE(
      4, output->verbose(CALL_INFO, 4, 0, "Generated a FENCE event.\n"));
}

void ArielCore::createExitEvent() {
  auto *xEv = new ArielExitEvent();
  coreQ->push(xEv);

  ARIEL_CORE_VERBOSE(
      4, output->verbose(CALL_INFO, 4, 0, "Generated an EXIT event.\n"));
}

bool ArielCore::isCoreHalted() const { return isHalted; }

bool ArielCore::isCoreStalled() const { return isStalled; }
...
void ArielCore::createGpuEvent(GpuApi_t API, CudaArguments CA) {
  ...
  ARIEL_CORE_VERBOSE(
      4, output->verbose(CALL_INFO, 4, 0, "Generated a CUDA event.\n"));
}

cudaMemcpyKind ArielCore::getKind() const { return kind; }

bool ArielCore::isGpuEx() const { return isGpu; }

bool ArielCore::getMidTransfer() const { return midTransfer; }

size_t ArielCore::getPageTransfer() const { return pageTransfer; }

size_t ArielCore::getTotalTransfer() const { return totalTransfer; }

size_t ArielCore::getPageAckTransfer() const { return pageAckTransfer; }

size_t ArielCore::getAckTransfer() const { return ackTransfer; }

size_t ArielCore::getRemainingPageTransfer() const {
  return remainingPageTransfer;
}

size_t ArielCore::getRemainingTransfer() const { return remainingTransfer; }

uint64_t ArielCore::getBaseAddress() const { return baseAddress; }

uint64_t ArielCore::getCurrentAddress() const { return currentAddress; }

uint8_t *ArielCore::getDataAddress() const { return dataAddress; }

uint8_t *ArielCore::getBaseDataAddress() const { return baseDataAddress; }

int ArielCore::getOpenTransactions() const {
  return maxPendingTransactions - pendingTransactions->size() -
         pendingGpuTransactions->size();
}
#endif
...
bool ArielCore::refillQueue() {
  ARIEL_CORE_VERBOSE(
      16, output->verbose(CALL_INFO, 16, 0,
                          "Refilling event queue for core %" PRIu32 "...\n",
                          coreID));
  ...
  while (coreQ->size() < maxQLength) {
    ARIEL_CORE_VERBOSE(
        16, output->verbose(
                CALL_INFO, 16, 0,
                "Attempting to fill events for core: %" PRIu32
                " current queue size=%" PRIu32 ", max length=%" PRIu32 "\n",
    ...
    if (!avail) {
      ARIEL_CORE_VERBOSE(
          32, output->verbose(CALL_INFO, 32, 0,
                              "Tunnel claims no data on core: %" PRIu32 "\n",
                              coreID));
      return false;
    }

    ARIEL_CORE_VERBOSE(32, output->verbose(CALL_INFO, 32, 0,
                                           "Tunnel reads data on core: %" PRIu32
                                           "\n",
                                           coreID));
    ...
    case ARIEL_OUTPUT_STATS:
      fprintf(stdout,
              "Performing statistics output at simulation time = %" PRIu64
              " cycles\n",
      ...
      while (ac.command != ARIEL_END_INSTRUCTION) {
        ...
        default:
          ...
          output->fatal(CALL_INFO, -1,
                        "Error: Ariel did not understand command (%d) provided "
                        "during instruction queue refill.\n",
                        (int)(ac.command));
          break;
        }
      }
    ...
    case ARIEL_ISSUE_TLM_MMAP:
      createMmapEvent(ac.mlm_mmap.fileID, ac.mlm_mmap.vaddr,
                      ac.mlm_mmap.alloc_len, ac.mlm_mmap.alloc_level,
                      ac.instPtr);
      break;

    case ARIEL_ISSUE_TLM_MAP:
      createAllocateEvent(ac.mlm_map.vaddr, ac.mlm_map.alloc_len,
                          ac.mlm_map.alloc_level, ac.instPtr);
      break;
...
void ArielCore::handleFreeEvent(ArielFreeEvent *rFE) {
  ARIEL_CORE_VERBOSE(
      4, output->verbose(
             CALL_INFO, 4, 0,
             "Core %" PRIu32
             " processing a free event (for virtual address=%" PRIu64 ")\n",
...
void ArielCore::handleReadRequest(ArielReadEvent *rEv) {
  ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0,
                                        "Core %" PRIu32
                                        " processing a read event...\n",
                                        coreID));

  const uint64_t readAddress = rEv->getAddress();
  const uint64_t readLength =
      std::min((uint64_t)rEv->getLength(),
               cacheLineSize); // Trim to cacheline size (occurs rarely for
                               // instructions such as xsave and fxsave)

  /* No longer neccessary due to trimming above
   * if(readLength > cacheLineSize) {
      output->verbose(CALL_INFO, 4, 0, "Potential error? request for a read of
  length=%" PRIu64 " is larger than cache line which is not allowed (coreID=%"
  PRIu32 ", cache line: %" PRIu64 "\n", readLength, coreID, cacheLineSize);
      return;
  }*/

  // NOTE: Physical and virtual addresses may not be aligned the same w.r.t.
  // line size if map-on-malloc is being used (arielinterceptcalls != 0), so use
  // physical offsets to determine line splits There is a chance that the
  // non-alignment causes an undetected bug if an access spans multiple malloc
  // regions that are contiguous in VA space but non-contiguous in PA space.
  // However, a single access spanning multiple malloc'd regions shouldn't
  // happen... Addresses mapped via first touch are always line/page aligned
  ...
  if ((addr_offset + readLength) <= cacheLineSize) {
    ARIEL_CORE_VERBOSE(
        4, output->verbose(CALL_INFO, 4, 0,
                           "Core %" PRIu32
                           " generating a non-split read request: Addr=%" PRIu64
                           " Length=%" PRIu64 "\n",
    ...
    ARIEL_CORE_VERBOSE(
        4, output->verbose(CALL_INFO, 4, 0,
                           "Core %" PRIu32 " issuing read, VAddr=%" PRIu64
                           ", Size=%" PRIu64 ", PhysAddr=%" PRIu64 "\n",
    ...
    ARIEL_CORE_VERBOSE(
        4, output->verbose(CALL_INFO, 4, 0,
                           "Core %" PRIu32
                           " generating a split read request: Addr=%" PRIu64
                           " Length=%" PRIu64 "\n",
                           coreID, readAddress, readLength));
    ...
    const uint64_t rightAddr =
        (readAddress + ((uint64_t)cacheLineSize)) - addr_offset;
    ...
    ARIEL_CORE_VERBOSE(
        4, output->verbose(CALL_INFO, 4, 0,
                           "Core %" PRIu32
                           " issuing split-address read, LeftVAddr=%" PRIu64
                           ", RightVAddr=%" PRIu64 ", LeftSize=%" PRIu64
                           ", RightSize=%" PRIu64 ", LeftPhysAddr=%" PRIu64
                           ", RightPhysAddr=%" PRIu64 "\n",
                           coreID, leftAddr, rightAddr, leftSize, rightSize,
                           physLeftAddr, physRightAddr));

    if (perform_checks > 0) {
      /*  if( (leftSize + rightSize) != readLength ) {
                  output->fatal(CALL_INFO, -4, "Core %" PRIu32 " read request
         for address %" PRIu64 ", length=%" PRIu64 ", split into left address=%"
         PRIu64 ", left size=%" PRIu64 ", right address=%" PRIu64 ", right
         size=%" PRIu64 " does not equal read length (cache line of length %"
         PRIu64 ")\n", coreID, readAddress, readLength, leftAddr, leftSize,
         rightAddr, rightSize, cacheLineSize);
              }*/

      if (((physLeftAddr + leftSize) % cacheLineSize) != 0) {
        output->fatal(CALL_INFO, -4,
                      "Error leftAddr=%" PRIu64 " + size=%" PRIu64
      ...
      /*if( ((physRightAddr + rightSize) % cacheLineSize) > cacheLineSize ) {
          output->fatal(CALL_INFO, -4, "Error rightAddr=%" PRIu64 " + size=%"
      PRIu64 " is not a multiple of cache line size: %" PRIu64 "\n", leftAddr,
      leftSize, cacheLineSize);
      }*/
    }
...
void ArielCore::handleWriteRequest(ArielWriteEvent *wEv) {
  ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0,
                                        "Core %" PRIu32
                                        " processing a write event...\n",
                                        coreID));

  const uint64_t writeAddress = wEv->getAddress();
  const uint64_t writeLength =
      std::min((uint64_t)wEv->getLength(),
               cacheLineSize); // Trim to cacheline size (occurs rarely for
                               // instructions such as xsave and fxsave)

  // No longer neccessary due to trimming above
  /*    if(writeLength > cacheLineSize) {
          output->verbose(CALL_INFO, 4, 0, "Potential error? request for a write
     of length=%" PRIu64 " is larger than cache line which is not allowed
     (coreID=%" PRIu32 ", cache line: %" PRIu64 "\n", writeLength, coreID,
  ...
  if ((addr_offset + writeLength) <= cacheLineSize) {
#ifdef HAVE_CUDA
    if (isGpuEx()) {
      // Save only the first physical address. Assume contiguous physical
      // memory.
    ...
    ARIEL_CORE_VERBOSE(
        ...
        output->verbose(CALL_INFO, 4, 0,
                        "Core %" PRIu32
                        " generating a non-split write request: Addr=%" PRIu64
                        " Length=%" PRIu64 "\n",
                        coreID, writeAddress, writeLength));

    ARIEL_CORE_VERBOSE(
        4, output->verbose(CALL_INFO, 4, 0,
                           "Core %" PRIu32 " issuing write, VAddr=%" PRIu64
                           ", Size=%" PRIu64 ", PhysAddr=%" PRIu64 "\n",
    ...
    if (writePayloads) {
      ...
      commitWriteEvent(physAddr, writeAddress, (uint32_t)writeLength, nullptr);
    }
  } else {
    ARIEL_CORE_VERBOSE(
        4, output->verbose(CALL_INFO, 4, 0,
                           "Core %" PRIu32
                           " generating a split write request: Addr=%" PRIu64
                           " Length=%" PRIu64 "\n",
                           coreID, writeAddress, writeLength));
    ...
    const uint64_t rightAddr =
        (writeAddress + ((uint64_t)cacheLineSize)) - addr_offset;
    ...
    ARIEL_CORE_VERBOSE(
        4, output->verbose(CALL_INFO, 4, 0,
                           "Core %" PRIu32
                           " issuing split-address write, LeftVAddr=%" PRIu64
                           ", RightVAddr=%" PRIu64 ", LeftSize=%" PRIu64
                           ", RightSize=%" PRIu64 ", LeftPhysAddr=%" PRIu64
                           ", RightPhysAddr=%" PRIu64 "\n",
                           coreID, leftAddr, rightAddr, leftSize, rightSize,
    ...
    if (perform_checks > 0) {
      /* if( (leftSize + rightSize) != writeLength ) {
          output->fatal(CALL_INFO, -4, "Core %" PRIu32 " write request for
      address %" PRIu64 ", length=%" PRIu64 ", split into left address=%" PRIu64
      ", left size=%" PRIu64 ", right address=%" PRIu64 ", right size=%" PRIu64
      " does not equal write length (cache line of length %" PRIu64 ")\n",
                      coreID, writeAddress, writeLength, leftAddr, leftSize,
      rightAddr, rightSize, cacheLineSize);
      }*/
    ...
    if (writePayloads) {
      ...
      commitWriteEvent(physLeftAddr, leftAddr, (uint32_t)leftSize, nullptr);
      commitWriteEvent(physRightAddr, rightAddr, (uint32_t)rightSize, nullptr);
    }
...
void ArielCore::handleMmapEvent(ArielMmapEvent *aEv) {
  memmgr->allocateMMAP(aEv->getAllocationLength(), aEv->getAllocationLevel(),
                       aEv->getVirtualAddress(), aEv->getInstructionPointer(),
                       aEv->getFileID(), coreID);
}

void ArielCore::handleAllocationEvent(ArielAllocateEvent *aEv) {
  output->verbose(CALL_INFO, 2, 0,
                  "Handling a memory allocation event, vAddr=%" PRIu64
                  ", length=%" PRIu64 ", at level=%" PRIu32
                  " with malloc ID=%" PRIu64 "\n",
                  aEv->getVirtualAddress(), aEv->getAllocationLength(),
                  aEv->getAllocationLevel(), aEv->getInstructionPointer());

  memmgr->allocateMalloc(aEv->getAllocationLength(), aEv->getAllocationLevel(),
                         aEv->getVirtualAddress(), aEv->getInstructionPointer(),
                         coreID);
}

void ArielCore::handleFlushEvent(ArielFlushEvent *flEv) {
  const auto virtualAddress = (uint64_t)flEv->getVirtualAddress();
  const auto readLength = (uint64_t)flEv->getLength();
...
void ArielCore::handleFenceEvent(ArielFenceEvent * /*fEv*/) {
  /*  Todo: Should we treat this like the Flush event, and require that the
   * Fence be put into a transaction queue?  */
  // Possibility A:
  fence();
  // Possibility B:
  // commitFenceEvent();
  ...
  if (gpu_enabled) {
    ...
    case GPU_MEMCPY: {
      ...
      if (getKind() == cudaMemcpyHostToDevice) {
        ...
        output->verbose(CALL_INFO, 16, 0,
                        "CUDA: D2H received first page from tunnel\n");

        // Take 4k (max) from tunnelD at a time
        size_t page_transfer =
            (getTotalTransfer() <= (1 << 12)) ? getTotalTransfer() : (1 << 12);
...
void ArielCore::handleGpuAckEvent(SST::Event *e) {
  ...
  if (ev->getType() == BalarComponent::EventType::RESPONSE) {
    if ((ev->API == GPU_MEMCPY_RET) &&
        (ev->CA.cuda_memcpy.kind == cudaMemcpyDeviceToHost)) {
      ...
      if (ev->API == GPU_MALLOC_RET) {
        ...
        std::cout << "Received ACK fatbin handle response "
                  << gc.fat_cubin_handle << std::endl;
      } else if (ev->API == GPU_MAX_BLOCK_RET) {
        gc.num_block = ev->CA.max_active_block.numBlock;
        std::cout << "Received ACK numblock response " << gc.num_block
                  << std::endl;
      }
      output->verbose(CALL_INFO, 16, 0, "CUDA: Ariel sent ACK\n");
      tunnelR->writeMessage(coreID, gc);
...
bool ArielCore::processNextEvent() {
  ...
  // Upon every call, check if the core is drained and we are fenced. If so,
  // unfence return true; /* Todo: reevaluate if this is needed */ Attempt to
  // refill the queue
  if (coreQ->empty()) {
  ...
  ARIEL_CORE_VERBOSE(8, output->verbose(CALL_INFO, 8, 0,
                                        "Processing next event in core %" PRIu32
                                        "...\n",
                                        coreID));
  ...
  case NOOP:
    ARIEL_CORE_VERBOSE(
        8, output->verbose(CALL_INFO, 8, 0,
                           "Core %" PRIu32 " next event is NOOP\n", coreID));
    statInstructionCount->addData(1);
    inst_count++;
  ...
  case READ_ADDRESS:
    ...
    if (pending_transaction_count < maxPendingTransactions) {
      ARIEL_CORE_VERBOSE(
          16, output->verbose(CALL_INFO, 16, 0,
                              "Found a read event, fewer pending transactions "
                              "than permitted so will process...\n"));
      statInstructionCount->addData(1);
      inst_count++;
      removeEvent = true;
      ...
      ARIEL_CORE_VERBOSE(
          ...
          output->verbose(
              CALL_INFO, 16, 0,
              "Pending transaction queue is currently full for core %" PRIu32
  ...
  case WRITE_ADDRESS:
    ...
    if (pending_transaction_count < maxPendingTransactions) {
      ARIEL_CORE_VERBOSE(
          16, output->verbose(CALL_INFO, 16, 0,
                              "Found a write event, fewer pending transactions "
                              "than permitted so will process...\n"));
      statInstructionCount->addData(1);
      inst_count++;
      removeEvent = true;
  ...
  case FREE:
    ARIEL_CORE_VERBOSE(
        8, output->verbose(CALL_INFO, 8, 0,
                           "Core %" PRIu32 " next event is FREE\n", coreID));
    removeEvent = true;
  ...
  case MALLOC:
    ARIEL_CORE_VERBOSE(
        8, output->verbose(CALL_INFO, 8, 0,
                           "Core %" PRIu32 " next event is MALLOC\n", coreID));
    removeEvent = true;
  ...
  case MMAP:
    ARIEL_CORE_VERBOSE(
        8, output->verbose(CALL_INFO, 8, 0,
                           "Core %" PRIu32 " next event is MMAP\n", coreID));
    removeEvent = true;
  ...
  case CORE_EXIT:
    ARIEL_CORE_VERBOSE(8, output->verbose(CALL_INFO, 8, 0,
                                          ...
                                          coreID));
    isHalted = true;
    std::cout << "CORE ID: " << coreID << " PROCESSED AN EXIT EVENT"
              << std::endl;
  ...
  case FLUSH:
    ARIEL_CORE_VERBOSE(
        8, output->verbose(CALL_INFO, 8, 0,
                           "Core %" PRIu32 " next event is a FLUSH\n", coreID));
    if (pending_transaction_count < maxPendingTransactions) {
      ARIEL_CORE_VERBOSE(
          16, output->verbose(CALL_INFO, 16, 0,
                              "Found a FLUSH event, fewer pending transactions "
                              "than permitted so will process..\n"));
      statInstructionCount->addData(1);
      inst_count++;
  ...
  case FENCE:
    ARIEL_CORE_VERBOSE(
        8, output->verbose(CALL_INFO, 8, 0,
                           "Core %" PRIu32 " next event is a FENCE\n", coreID));
    if (!isCoreFenced()) { // If core is fenced, drop this fence - they can be
                           // merged
  ...
  default:
    output->fatal(CALL_INFO, -4,
                  "Unknown event type has arrived on core %" PRIu32 "\n",
                  coreID);
    break;
  }

  // If the event has actually been processed this cycle then remove it from the
  // queue
  if (removeEvent) {
    ARIEL_CORE_VERBOSE(
        8,
        output->verbose(CALL_INFO, 8, 0,
                        "Removing event from pending queue, there are %" PRIu32
                        " events in the queue before deletion.\n",
                        (uint32_t)coreQ->size()));
    coreQ->pop();
    ...
    ARIEL_CORE_VERBOSE(
        8, output->verbose(
               CALL_INFO, 8, 0,
               "Event removal was not requested, pending transaction queue "
               "length=%" PRIu32 ", maximum transactions: %" PRIu32 "\n",
