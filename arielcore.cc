...
ArielCore::ArielCore(ComponentId_t id, ArielTunnel *tunnel,
    ...
    output->verbose(CALL_INFO, 2, 0, "Creating core with ID %"
    PRIu32
    ...
    memmgr->registerInterruptHandler(coreID,
                                     new ArielMemoryManager::InterruptHandler<ArielCore>(this,
                                                                                         &ArielCore::handleInterrupt));
...
void ArielCore::commitReadEvent(const uint64_t address,
    ...
    if (length > 0) {
        ...
        pendingTransactions->insert(
            std::pair<SimpleMem::Request::id_t, SimpleMem::Request *>(req->id, req));
...
void ArielCore::commitWriteEvent(const uint64_t address,
    ...
    if (length > 0) {
        SimpleMem::Request *req = new SimpleMem::Request(SimpleMem::Request::Write, address,
                                                         length);
        req->setVirtualAddress(virtAddress);
        ...
        if (writePayloads) {
            if (verbosity >= 16) {
                ...
                output->verbose(CALL_INFO, 16, 0, "Write-Payload: Len=%"
                PRIu32
                ", Data={ %s }\n",
...
void ArielCore::commitFlushEvent(const uint64_t address,
    ...
    if (length > 0) {
        ...
        SimpleMem::Request *req = new SimpleMem::Request(SimpleMem::Request::FlushLineInv, address,
                                                         length);
...
void ArielCore::handleEvent(SimpleMem::Request *event) {
    ...
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %"
        PRIu32
    ...
    if (find_entry != pendingTransactions->end()) {
        ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0,
                                              "Correctly identified event in pending transactions, removing from list, before there are: %"
            PRIu32
        ...
        output->fatal(CALL_INFO, -4, "Memory event response to core: %"
        PRIu32
...
bool ArielCore::handleInterrupt(ArielMemoryManager::InterruptAction action) {
    ...
    switch (action) {
        ...
        case ArielMemoryManager::InterruptAction::UNSTALL:
            ...
            break;
        default:
            output->fatal(CALL_INFO, -4, "Received an unknown interrupt on core %"
            PRIu32
            "\n", coreID);
    }
...
void ArielCore::fence() {
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core: %"
        PRIu32
        " FENCE:  Current pending transaction count: %"
        PRIu32
        " (%"
        PRIu32
...
void ArielCore::unfence() {
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core: %"
        PRIu32
        " UNFENCE:  Current pending transaction count: %"
        PRIu32
        " (%"
        PRIu32
        ")\n", coreID, pending_transaction_count, maxPendingTransactions));
...
void ArielCore::handleSwitchPoolEvent(ArielSwitchPoolEvent *aSPE) {
    ARIEL_CORE_VERBOSE(2, output->verbose(CALL_INFO, 2, 0, "Core: %"
        PRIu32
        " set default memory pool to: %"
        PRIu32
        "\n", coreID, aSPE->getPool()));
...
void ArielCore::createSwitchPoolEvent(uint32_t newPool) {
    ...
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Generated a switch pool event on core %"
        PRIu32
        ", new level is: %"
        PRIu32
        "\n", coreID, newPool));
...
void ArielCore::createNoOpEvent() {
    ...
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Generated a No Op event on core %"
        PRIu32
        "\n", coreID));
}
...
void ArielCore::createReadEvent(uint64_t address, uint32_t length) {
    ...
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Generated a READ event, addr=%"
        PRIu64
        ", length=%"
        PRIu32
        "\n", address, length));
...
void ArielCore::createAllocateEvent(uint64_t vAddr, uint64_t length, uint32_t level,
    ...
    ARIEL_CORE_VERBOSE(2,
                       output->verbose(CALL_INFO, 2, 0, "Generated an allocate event, vAddr(map)=%"
                           PRIu64
                           ", length=%"
                           PRIu64
                           " in level %"
...
void ArielCore::createMmapEvent(uint32_t fileID, uint64_t vAddr, uint64_t length, uint32_t level,
                                uint64_t instPtr) {
    ArielMmapEvent *ev = new ArielMmapEvent(fileID, vAddr, length, level, instPtr);
    coreQ->push(ev);

    ARIEL_CORE_VERBOSE(2, output->verbose(CALL_INFO, 2, 0, "Generated an mmap event, vAddr(map)=%"
        PRIu64
        ", length=%"
        PRIu64
        " in level %"
...
void ArielCore::createFreeEvent(uint64_t vAddr) {
    ...
    ARIEL_CORE_VERBOSE(2, output->verbose(CALL_INFO, 2, 0,
                                          "Generated a free event for virtual address=%"
        PRIu64
        "\n", vAddr));
}
...
void ArielCore::createWriteEvent(uint64_t address, uint32_t length, const uint8_t *payload) {
    ...
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Generated a WRITE event, addr=%"
        PRIu64
        ", length=%"
        PRIu32
        "\n", address, length));
...
bool ArielCore::refillQueue() {
    ARIEL_CORE_VERBOSE(16, output->verbose(CALL_INFO, 16, 0, "Refilling event queue for core %"
        PRIu32
        "...\n", coreID));

    while (coreQ->size() < maxQLength) {
        ARIEL_CORE_VERBOSE(16,
                           output->verbose(CALL_INFO, 16, 0, "Attempting to fill events for core: %"
                               PRIu32
                               " current queue size=%"
                               PRIu32
                               ", max length=%"
        ...
        if (!avail) {
            ARIEL_CORE_VERBOSE(32,
                               output->verbose(CALL_INFO, 32, 0, "Tunnel claims no data on core: %"
                                   PRIu32
                                   "\n", coreID));
            return false;
        }

        ARIEL_CORE_VERBOSE(32, output->verbose(CALL_INFO, 32, 0, "Tunnel reads data on core: %"
            PRIu32
            "\n", coreID));
        ...
        switch (ac.command) {
            case ARIEL_OUTPUT_STATS:
                fprintf(stdout, "Performing statistics output at simulation time = %"
                PRIu64
                    ...
                    switch (ac.command) {
                        ...
                        default:
                            ...
                            output->fatal(CALL_INFO, -1,
                                          "Error: Ariel did not understand command (%d) provided during instruction queue refill.\n",
                                          (int) (ac.command));
                            break;
                    }
                }
            ...
            case ARIEL_ISSUE_TLM_MMAP:
                createMmapEvent(ac.mlm_mmap.fileID, ac.mlm_mmap.vaddr, ac.mlm_mmap.alloc_len,
                                ac.mlm_mmap.alloc_level, ac.instPtr);
                break;
...
void ArielCore::handleFreeEvent(ArielFreeEvent *rFE) {
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %"
        PRIu32
        " processing a free event (for virtual address=%"
        PRIu64
...
void ArielCore::handleReadRequest(ArielReadEvent *rEv) {
    ...
    const uint64_t readLength = std::min((uint64_t) rEv->getLength(),
                                         cacheLineSize); // Trim to cacheline size (occurs rarely for instructions such as xsave and fxsave)
    ...
    if ((addr_offset + readLength) <= cacheLineSize) {
        ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %"
            PRIu32
            " generating a non-split read request: Addr=%"
            PRIu64
            " Length=%"
            PRIu64
            "\n",
        ...
        ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %"
            PRIu32
            " generating a split read request: Addr=%"
            PRIu64
            " Length=%"
            PRIu64
            "\n",
        ...
        ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %"
            PRIu32
            " issuing split-address read, LeftVAddr=%"
            PRIu64
            ", RightVAddr=%"
            PRIu64
            ", LeftSize=%"
        ...
        if (perform_checks > 0) {
            ...
            if (((physLeftAddr + leftSize) % cacheLineSize) != 0) {
                output->fatal(CALL_INFO, -4, "Error leftAddr=%"
                PRIu64
                " + size=%"
                PRIu64
...
void ArielCore::handleWriteRequest(ArielWriteEvent *wEv) {
    ...
    const uint64_t writeLength = std::min((uint64_t) wEv->getLength(),
                                          cacheLineSize); // Trim to cacheline size (occurs rarely for instructions such as xsave and fxsave)
...
/*    if(writeLength > cacheLineSize) {
    ...
    if ((addr_offset + writeLength) <= cacheLineSize) {
        ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %"
            PRIu32
            " generating a non-split write request: Addr=%"
            PRIu64
            " Length=%"
            PRIu64
            "\n",
        ...
        ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %"
            PRIu32
            " generating a split write request: Addr=%"
            PRIu64
            " Length=%"
            PRIu64
            "\n",
        ...
        ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %"
            PRIu32
            " issuing split-address write, LeftVAddr=%"
            PRIu64
            ", RightVAddr=%"
            PRIu64
            ", LeftSize=%"
...
void ArielCore::handleAllocationEvent(ArielAllocateEvent *aEv) {
    output->verbose(CALL_INFO, 2, 0, "Handling a memory allocation event, vAddr=%"
    PRIu64
    ", length=%"
    PRIu64
    ", at level=%"
    ...
    memmgr->allocateMalloc(aEv->getAllocationLength(), aEv->getAllocationLevel(),
                           aEv->getVirtualAddress(), aEv->getInstructionPointer(), coreID);
}
...
bool ArielCore::processNextEvent() {
    ...
    ARIEL_CORE_VERBOSE(8, output->verbose(CALL_INFO, 8, 0, "Processing next event in core %"
        PRIu32
        "...\n", coreID));
    ...
    switch (nextEvent->getEventType()) {
        ...
        ARIEL_CORE_VERBOSE(8, output->verbose(CALL_INFO, 8, 0, "Core %"
            ...
            output->verbose(CALL_INFO, 2, 0, "Core %"
            PRIu32
            " has called exit.\n", coreID);
            return true;
        ...
        ARIEL_CORE_VERBOSE(8, output->verbose(CALL_INFO, 8, 0, "Core %"
            PRIu32
            " next event is a FLUSH\n", coreID));
            if (pending_transaction_count < maxPendingTransactions) {
                ARIEL_CORE_VERBOSE(16, output->verbose(CALL_INFO, 16, 0,
                                                       "Found a FLUSH event, fewer pending transactions than permitted so will process..\n"));
                statInstructionCount->addData(1);
                inst_count++;
        ...
        default:
            output->fatal(CALL_INFO, -4, "Unknown event type has arrived on core %"
            PRIu32
            "\n", coreID);
            break;
    }
    ...
    if (removeEvent) {
        ARIEL_CORE_VERBOSE(8, output->verbose(CALL_INFO, 8, 0,
                                              "Removing event from pending queue, there are %"
            PRIu32
        ...
        ARIEL_CORE_VERBOSE(8, output->verbose(CALL_INFO, 8, 0,
                                              "Event removal was not requested, pending transaction queue length=%"
            PRIu32
...
void ArielCore::tick() {
    ...
    if (!isHalted) {
        ARIEL_CORE_VERBOSE(16, output->verbose(CALL_INFO, 16, 0, "Ticking core id %"
            PRIu32
            "\n", coreID));
        updateCycle = false;
