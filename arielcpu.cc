...
/** This section of code preserves backward compability from the old memorymanager parameters to the new subcomponent structure. */
    ...
    if (found) {
        output->verbose(CALL_INFO, 0, 0,
                        "WARNING - ariel parameter name change: change 'vtop' to 'memmgr.vtop' in your input. The old parameter will continue to work for now but may not be supported in future releases.\n");
    ...
    if (found) {
        output->verbose(CALL_INFO, 0, 0,
                        "WARNING - ariel parameter name change: change 'pagemappolicy' to 'memmgr.pagemappolicy' in your input. The old parameter will continue to work for now but may not be supported in future releases.\n");
    ...
    if (found) {
        output->verbose(CALL_INFO, 0, 0,
                        "WARNING - ariel parameter name change: change 'translatecacheentries' to 'memmgr.translatecacheentries' in your input. The old parameter will continue to work for now but may not be supported in future releases.\n");
    ...
    if (found) {
        output->verbose(CALL_INFO, 0, 0,
                        "WARNING - ariel parameter name change: change 'memorylevels' to 'memmgr.memorylevels' in your input. The old parameter will continue to work for now but may not be supported in future releases.\n");
    ...
    if (found) {
        output->verbose(CALL_INFO, 0, 0,
                        "WARNING - ariel parameter name change: change 'defaultlevel' to 'memmgr.defaultlevel' in your input. The old parameter will continue to work for now but may not be supported in future releases.\n");
    ...
    for (uint32_t i = 0; i < memLevels; i++) {
        ...
        if (found) {
            output->verbose(CALL_INFO, 0, 0,
                            "WARNING - ariel parameter name change: change '%s' to 'memmgr.%s' in your input. The old parameter will continue to work for now but may not be supported in future releases.\n",
                            level_buffer, level_buffer);
            sprintf(level_buffer, "memmgr.pagesize%"
            PRIu32, i);
        ...
        if (found) {
            output->verbose(CALL_INFO, 0, 0,
                            "WARNING - ariel parameter name change: change '%s' to 'memmgr.%s' in your input. The old parameter will continue to work for now but may not be supported in future releases.\n",
                            level_buffer, level_buffer);
            sprintf(level_buffer, "memmgr.pagecount%"
            PRIu32, i);
            params.find<std::string>(level_buffer, "", found);
            if (!found) params.insert(level_buffer, paramStr, true);
        }
        ...
        if (found) {
            output->verbose(CALL_INFO, 0, 0,
                            "WARNING - ariel parameter name change: change '%s' to 'memmgr.%s' in your input. The old parameter will continue to work for now but may not be supported in future releases.\n",
                            level_buffer, level_buffer);
            sprintf(level_buffer, "memmgr.pagepopulate%"
            PRIu32, i);
            params.find<std::string>(level_buffer, "", found);
            if (!found) params.insert(level_buffer, paramStr, true);
        }
...
/** End memory manager subcomponent parameter translation */
    ...
    if ("" == ariel_tool) {
        output->fatal(CALL_INFO, -1,
                      "The arieltool parameter specifying which PIN tool to run was not specified\n");
    }
    ...
    if ("" == executable) {
        output->fatal(CALL_INFO, -1,
                      "The input deck did not specify an executable to be run against PIN\n");
    }

    uint32_t app_argc = (uint32_t) params.find<uint32_t>("appargcount", 0);
    output->verbose(CALL_INFO, 1, 0, "Model specifies that there are %"
    PRIu32
    ...
    switch (intercept_mem_allocations) {
        case 0:
            output->verbose(CALL_INFO, 1, 0,
                            "Interception and instrumentation of multi-level memory and malloc/free calls is DISABLED.\n");
            break;
        default:
            output->verbose(CALL_INFO, 1, 0,
                            "Interception and instrumentation of multi-level memory and malloc/free calls is ENABLED.\n");
            break;
    }
    ...
    for (uint32_t aa = 0; aa < launch_param_count; aa++) {
        sprintf(param_name_buffer, "launchparam%"
        PRIu32, aa);
        std::string launch_p = params.find<std::string>(param_name_buffer, "");

        if ("" == launch_p) {
            output->fatal(CALL_INFO, -1, "Error: launch parameter %"
            PRId32
            " is empty string, this must be set to a value.\n",
                aa);
        }
    ...
    execute_args[arg++] = (char *) malloc(sizeof(char) * 8);
    sprintf(execute_args[arg - 1], "%"
    PRIu32, pin_startup_mode);
    execute_args[arg++] = const_cast<char *>("-m");
    execute_args[arg++] = (char *) malloc(sizeof(char) * 8);
    sprintf(execute_args[arg - 1], "%"
    PRIu32, intercept_mem_allocations);
    execute_args[arg++] = const_cast<char *>("-k");
    execute_args[arg++] = (char *) malloc(sizeof(char) * 8);
    sprintf(execute_args[arg - 1], "%"
    PRIu32, keep_malloc_stack_trace);
    ...
    for (uint32_t aa = 0; aa < app_argc; ++aa) {
        sprintf(argv_buffer, "apparg%"
        PRIu32, aa);
        std::string argv_i = params.find<std::string>(argv_buffer, "");

        output->verbose(CALL_INFO, 1, 0, "Found application argument %"
        PRIu32
        " (%s) = %s\n",
    ...
    if (pin_env_count > -1) {
        ...
        for (int32_t next_env_param = 0; next_env_param < pin_env_count; next_env_param++) {
            sprintf(env_name_buffer, "envparamname%"
            PRId32, next_env_param);
            ...
            sprintf(env_name_buffer, "envparamval%"
            PRId32, next_env_param);
    ...
    TimeConverter *timeconverter = registerClock(cpu_clock, new Clock::Handler<ArielCPU>(this,
                                                                                         &ArielCPU::tick));
    ...
    for (uint32_t i = 0; i < core_count; ++i) {
        ...
        cpu_cores.push_back(
            loadComponentExtension<ArielCore>(tunnel, i, maxPendingTransCore, output,
                                              maxIssuesPerCycle, maxCoreQueueLen, cacheLineSize,
    ...
    if (mem) {
        if (!mem->isAllPopulated())
            output->fatal(CALL_INFO, -1,
                          "%s, Error: loading 'memory' subcomponents. All subcomponent slots from 0 to core_count must be populated. Check your input config for non-populated slots\n",
                          getName().c_str());

        if (mem->getMaxPopulatedSlotNumber() != core_count - 1)
            output->fatal(CALL_INFO, -1,
                          "%s, Error: Loading 'memory' subcomponents and the number of subcomponents does not match the number of cores. Cores: %u, SubComps: %u. Check your input config.\n",
                          getName().c_str(), core_count, mem->getMaxPopulatedSlotNumber());

        for (int i = 0; i < core_count; i++) {
            cpu_to_cache_links.push_back(
                mem->create<Interfaces::SimpleMem>(i, ComponentInfo::INSERT_STATS, timeconverter,
        ...
        for (int i = 0; i < core_count; i++) {
            ...
            cpu_to_cache_links.push_back(
                loadAnonymousSubComponent<Interfaces::SimpleMem>("memHierarchy.memInterface",
                                                                 "memory", i,
                                                                 ComponentInfo::SHARE_PORTS |
                                                                 ComponentInfo::INSERT_STATS, par,
                                                                 timeconverter,
                                                                 new SimpleMem::Handler<ArielCore>(
                                                                     cpu_cores[i],
                                                                     &ArielCore::handleEvent)));
            cpu_cores[i]->setCacheLink(cpu_to_cache_links[i]);
        }
...
void ArielCPU::init(unsigned int phase) {
    if (phase == 0) {
        ...
        output->verbose(CALL_INFO, 1, 0,
                        "Returned from launching PIN.  Waiting for child to attach.\n");
...
void ArielCPU::finish() {
    ...
    output->verbose(CALL_INFO, 1, 0, "Completed at: %"
    PRIu64
