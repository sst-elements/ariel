...
ArielCPU::ArielCPU(ComponentId_t id, Params &params) : Component(id) {

  int verbosity = params.find<int>("verbose", 0);
  output = new SST::Output("ArielComponent[@f:@l:@p] ", verbosity, 0,
                           SST::Output::STDOUT);
  ...
  uint32_t perform_checks =
      (uint32_t)params.find<uint32_t>("checkaddresses", 0);
  output->verbose(CALL_INFO, 1, 0, "Configuring for check addresses = %s\n",
                  (perform_checks > 0) ? "yes" : "no");

  /** This section of code preserves backward compability from the old
   * memorymanager parameters to the new subcomponent structure. */
  ...
  if (found) {
    output->verbose(
        CALL_INFO, 0, 0,
        "WARNING - ariel parameter name change: change 'vtop' to 'memmgr.vtop' "
        "in your input. The old parameter will continue to work for now but "
        "may not be supported in future releases.\n");
    // Check if both parameters are defined - if so, ignore the old one
    params.find<bool>("memmgr.vtop", nullptr, found);
    if (!found)
      params.insert("memmgr.vtop", paramStr, true);
  }
  ...
  if (found) {
    output->verbose(
        CALL_INFO, 0, 0,
        "WARNING - ariel parameter name change: change 'pagemappolicy' to "
        "'memmgr.pagemappolicy' in your input. The old parameter will continue "
        "to work for now but may not be supported in future releases.\n");
    params.find<std::string>("memmgr.pagemappolicy", "", found);
    if (!found)
      params.insert("memmgr.pagemappolicy", paramStr, true);
  }

  paramStr = params.find<std::string>("translatecacheentries", "", found);
  if (found) {
    output->verbose(CALL_INFO, 0, 0,
                    "WARNING - ariel parameter name change: change "
                    "'translatecacheentries' to 'memmgr.translatecacheentries' "
                    "in your input. The old parameter will continue to work "
                    "for now but may not be supported in future releases.\n");
    params.find<std::string>("memmgr.translatecacheentries", "", found);
    if (!found)
      params.insert("memmgr.translatecacheentries", paramStr, true);
  }
  paramStr = params.find<std::string>("memorylevels", "", found);
  if (found) {
    output->verbose(
        CALL_INFO, 0, 0,
        "WARNING - ariel parameter name change: change 'memorylevels' to "
        "'memmgr.memorylevels' in your input. The old parameter will continue "
        "to work for now but may not be supported in future releases.\n");
    params.find<std::string>("memmgr.memorylevels", "", found);
    if (!found)
      params.insert("memmgr.memorylevels", paramStr, true);
  }
  paramStr = params.find<std::string>("defaultlevel", "", found);
  if (found) {
    output->verbose(
        CALL_INFO, 0, 0,
        "WARNING - ariel parameter name change: change 'defaultlevel' to "
        "'memmgr.defaultlevel' in your input. The old parameter will continue "
        "to work for now but may not be supported in future releases.\n");
    params.find<std::string>("memmgr.defaultlevel", "", found);
    if (!found)
      params.insert("memmgr.defaultlevel", paramStr, true);
  }
  uint32_t memLevels = params.find<uint32_t>("memmgr.memorylevels", 1, found);
  if (!found)
    memLevels = params.find<int>("memorylevels", 1);
  ...
  for (uint32_t i = 0; i < memLevels; i++) {
    ...
    if (found) {
      output->verbose(
          CALL_INFO, 0, 0,
          "WARNING - ariel parameter name change: change '%s' to 'memmgr.%s' "
          "in your input. The old parameter will continue to work for now but "
          "may not be supported in future releases.\n",
      ...
      if (!found)
        params.insert(level_buffer, paramStr, true);
    }
  ...
  std::string memorymanager =
      params.find<std::string>("memmgr", "ariel.MemoryManagerSimple");
  if (nullptr !=
      (memmgr = loadUserSubComponent<ArielMemoryManager>("memmgr"))) {
    ...
    if (memorymanager == "ariel.MemoryManagerSimple" && memLevels > 1) {
      output->verbose(CALL_INFO, 1, 0,
                      "Warning - the default 'ariel.MemoryManagerSimple' does "
                      "not support multiple memory levels. Configuring anyways "
                      "but memorylevels will be 1.\n");
    ...
    memmgr = loadAnonymousSubComponent<ArielMemoryManager>(
        memorymanager, "memmgr", 0,
        ComponentInfo::SHARE_STATS | ComponentInfo::INSERT_STATS, mmParams);
    if (nullptr == memmgr)
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Memory manager construction is completed.\n");

  uint32_t maxIssuesPerCycle =
      (uint32_t)params.find<uint32_t>("maxissuepercycle", 1);
  uint32_t maxCoreQueueLen =
      (uint32_t)params.find<uint32_t>("maxcorequeue", 64);
  uint32_t maxPendingTransCore =
      (uint32_t)params.find<uint32_t>("maxtranscore", 16);
  ...
  frontend = loadUserSubComponent<ArielFrontend>(
      "frontend", ComponentInfo::SHARE_NONE, core_count, maxCoreQueueLen,
      memmgr->getDefaultPool());
  if (!frontend) {
    // ariel.frontend.pin points to either pin2 or pin3 based on sst-elements
    // configuration
    frontend = loadAnonymousSubComponent<ArielFrontend>(
        "ariel.frontend.pin", "frontend", 0,
        ComponentInfo::INSERT_STATS | ComponentInfo::SHARE_STATS, params,
        core_count, maxCoreQueueLen, memmgr->getDefaultPool());
  }
  if (!frontend)
    output->fatal(CALL_INFO, -1,
                  "%s, Error: Loading frontend subcomponent failed\n",
                  getName().c_str());
  ...
  TimeConverter *timeconverter = registerClock(
      cpu_clock, new Clock::Handler<ArielCPU>(this, &ArielCPU::tick));
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Creating processor cores and cache links...\n");

  output->verbose(CALL_INFO, 1, 0, "Configuring cores and cache links...\n");
  for (uint32_t i = 0; i < core_count; ++i) {
    cpu_cores.push_back(loadComponentExtension<ArielCore>(
        tunnel,
#ifdef HAVE_CUDA
        tunnelR, tunnelD,
#endif
  ...
  if (mem) {
    if (!mem->isAllPopulated())
      output->fatal(
          CALL_INFO, -1,
          "%s, Error: loading 'memory' subcomponents. All subcomponent slots "
          "from 0 to core_count must be populated. Check your input config for "
          "non-populated slots\n",
          getName().c_str());

    if (mem->getMaxPopulatedSlotNumber() != core_count - 1)
      output->fatal(CALL_INFO, -1,
                    "%s, Error: Loading 'memory' subcomponents and the number "
                    "of subcomponents does not match the number of cores. "
                    "Cores: %u, SubComps: %u. Check your input config.\n",
                    getName().c_str(), core_count,
                    mem->getMaxPopulatedSlotNumber());

    for (int i = 0; i < core_count; i++) {
      cpu_to_cache_links.push_back(mem->create<Interfaces::SimpleMem>(
          i, ComponentInfo::INSERT_STATS, timeconverter,
          new SimpleMem::Handler<ArielCore>(cpu_cores[i],
                                            &ArielCore::handleEvent)));
      cpu_cores[i]->setCacheLink(cpu_to_cache_links[i]);
    }
  } else {
    // Load from here not the user one; let the subcomponent have our port
    // (cache_link)
    char *link_buffer = (char *)malloc(sizeof(char) * 256);
    for (int i = 0; i < core_count; i++) {
      Params par;
      par.insert("port", "cache_link_" + std::to_string(i));
      cpu_to_cache_links.push_back(
          loadAnonymousSubComponent<Interfaces::SimpleMem>(
              "memHierarchy.memInterface", "memory", i,
              ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, par,
              timeconverter,
              new SimpleMem::Handler<ArielCore>(cpu_cores[i],
                                                &ArielCore::handleEvent)));
      cpu_cores[i]->setCacheLink(cpu_to_cache_links[i]);
      ...
      if (gpu_enabled) {
        ...
        cpu_to_gpu_links.push_back(configureLink(
            link_buffer, new Event::Handler<ArielCore>(
                             cpu_cores[i], &ArielCore::handleGpuAckEvent)));
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Completed initialization of the Ariel CPU.\n");
  fflush(stdout);
}
...
bool ArielCPU::tick(SST::Cycle_t /*cycle*/) {
  ...
  output->verbose(CALL_INFO, 16, 0,
                  "Main processor tick, will issue to individual cores...\n");
