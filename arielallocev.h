...
public:
  ArielAllocateEvent(uint64_t vAddr, uint64_t len, uint32_t lev, uint64_t ip)
      : virtualAddress(vAddr), allocateLength(len), level(lev), instPtr(ip) {}
...
public:
  ArielMmapEvent(uint32_t fileID, uint64_t vAddr, uint64_t len, uint32_t lev,
                 uint64_t ip)
      : FileID(fileID), virtualAddress(vAddr), allocateLength(len), level(lev),
        instPtr(ip) {}
