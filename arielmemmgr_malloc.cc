...
ArielMemoryManagerMalloc::ArielMemoryManagerMalloc(ComponentId_t id,
                                                   Params &params)
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Configuring for %" PRIu32
                  " memory levels; default level is %" PRIu32 ".\n",
                  memoryLevels, defaultLevel);

  // Configure each memory level's free page pool
  freePages = (std::deque<uint64_t> **)malloc(sizeof(std::deque<uint64_t> *) *
                                              memoryLevels);
  ...
  for (uint32_t i = 0; i < memoryLevels; ++i) {
    ...
    output->verbose(CALL_INFO, 2, 0,
                    "Level %" PRIu32 " page size is %" PRIu64 "\n", i,
                    pageSizes[i]);
    ...
    output->verbose(CALL_INFO, 2, 0,
                    "Level %" PRIu32 " page count is %" PRIu64 "\n", i,
                    pageCount);
    ...
    output->verbose(CALL_INFO, 2, 0,
                    "Level %" PRIu32
                    " usable (free) page queue contains %" PRIu32 " entries\n",
    ...
    if (popFilePath != "") {
      output->verbose(CALL_INFO, 1, 0,
                      "Populating page tables for level %" PRIu32
                      " from %s...\n",
                      i, popFilePath.c_str());
    ...
    statBytesFree.push_back(
        registerStatistic<uint64_t>("bytes_freed_from_pool", level_buffer));
    statDemandAllocs.push_back(
        registerStatistic<uint64_t>("demand_page_allocs", level_buffer));
  }

  free(level_buffer);
}

ArielMemoryManagerMalloc::~ArielMemoryManagerMalloc() = default;

void ArielMemoryManagerMalloc::setDefaultPool(uint32_t pool) {
  defaultLevel = pool;
}

uint32_t ArielMemoryManagerMalloc::getDefaultPool() { return defaultLevel; }

/*
 *  Determine whether there are enough free pages in a level for an allocation
 * request
 */
bool ArielMemoryManagerMalloc::canAllocateInLevel(const uint64_t size,
                                                  const uint32_t level) {
  int pageCount =
      size / pageSizes[level] + ((size % pageSizes[level] == 0) ? 0 : 1);
  return freePages[level]->size() >= pageCount;
}

void ArielMemoryManagerMalloc::allocate(const uint64_t size,
                                        const uint32_t level,
                                        const uint64_t virtualAddress) {
  if (level >= memoryLevels) {
    output->fatal(CALL_INFO, -1,
                  "Requested memory allocation of %" PRIu64
                  " bytes, in level: %" PRIu32 ", but only have: %" PRIu32
                  " levels.\n",
                  size, level, memoryLevels);
  }

  output->verbose(CALL_INFO, 4, 0,
                  "Requesting a memory allocation of %" PRIu64
                  " bytes, in level: %" PRIu32 ", Virtual mapping=%" PRIu64
                  "\n",
  ...
  // We will do all of our allocation based on whole pages, inefficient maybe
  ...
  for (uint64_t bytesLeft = 0; bytesLeft < roundedSize; bytesLeft += pageSize) {
    if (freePages[level]->empty()) {
      output->verbose(CALL_INFO, 4, 0,
                      "Requesting a memory allocation at level: %" PRIu32
                      " which will fail due to not having enough free pages\n",
                      level);
      for (uint32_t i = 0; i < memoryLevels; ++i) {
        output->verbose(CALL_INFO, -1, 0,
                        "Free pages at level %" PRIu32 " : %" PRIu64 "\n", i,
                        static_cast<uint64_t>(freePages[i]->size()));
      }
      output->fatal(CALL_INFO, -1,
                    "Requested a memory allocation at level: %" PRIu32
                    " of size %" PRIu64
    ...
    pageTables[level]->insert(
        std::pair<uint64_t, uint64_t>(nextVirtPage, nextPhysPage));

    output->verbose(CALL_INFO, 4, 0,
                    "Allocating memory page, physical page=%" PRIu64
                    ", virtual page=%" PRIu64 "\n",
  ...
  output->verbose(CALL_INFO, 4, 0,
                  "Request leaves: %" PRIu32 " free pages at level: %" PRIu32
                  "\n",
                  (uint32_t)freePages[level]->size(), level);

  // Record the complete entry in the allocation table (what we allocated in
  // size against the virtual address) this means we know how much to free and
  // can translate the address successfully.
  pageAllocations[level]->insert(
      std::pair<uint64_t, uint64_t>(virtualAddress, roundedSize));
}

/*
 *  Note on allocation alignment:
 *  Physical and virtual mapping may not be aligned the same (have different
 * page/cache-line offsets). This happens because the VA assigned by the OS may
 * be different than the VA that would have been assigned in an MLM environment.
 *  For example the app allocates location A to VA 0 and location B to VA 8, but
 * in simulation we'd like A to be in memory pool 0 and B to be in pool 1. Then
 * A & B share a virtual cache block but not a simulated physical cache block.
 *
 *  Therefore, Ariel should immediately translate virtual to physical addresses
 * and ALWAYS use physical addresses when computing offsets.
 */
bool ArielMemoryManagerMalloc::allocateMalloc(
    const uint64_t size, const uint32_t level, const uint64_t virtualAddress,
    const uint64_t /*instructionPointer*/, const uint32_t /*thread*/) {
  output->verbose(CALL_INFO, 4, 0,
                  "Allocate malloc received. VA: %" PRIu64 ". Size: %" PRIu64
                  ". Level: %" PRIu32 ".\n",
                  virtualAddress, size, level);

  // Check whether a malloc mapping already exists (i.e., we missed a free)
  auto it = mallocTranslations.upper_bound(virtualAddress);
  ...
  if (it != mallocTranslations.end() && it->first <= virtualAddress) {
    ...
    if (virtualAddress <
        primaryAddr + (mallocInformation.find(primaryAddr)->second).size) {
      output->verbose(CALL_INFO, 4, 0,
                      "Found conflicting malloc, freeing address %" PRIu64 "\n",
                      primaryAddr);
      freeMalloc(primaryAddr);
    }
  }

  // Allocate new page(s). Round malloc to nearest whole page TODO fix so we can
  // map partial pages -> needs a local VA->Ariel_VA mapping
  ...
  if (freePages[level]->size() < pageCount) {
    output->verbose(
        CALL_INFO, 4, 0,
        "Requested memory cannot be allocated, not enough pages. Have: %" PRIu64
        ", Need: %" PRIu64 "\n",
  ...
  // Allocate the pages
  auto *virtualPages = new std::unordered_set<uint64_t>;
  uint64_t nextVirtPage = virtualAddress;
  ...
  output->verbose(CALL_INFO, 4, 0,
                  "Malloc mapped %" PRIu64 " to [%" PRIu64 ", %" PRIu64
                  "] (%" PRIu64 " pages).\n",
                  virtualAddress, firstPhysAddr, lastPhysAddr, pageCount);

  // Record malloc
  mallocInformation.insert(
      std::make_pair(virtualAddress, mallocInfo(size, level, virtualPages)));
...
void ArielMemoryManagerMalloc::freeMalloc(const uint64_t virtualAddress) {
  ...
  auto it = mallocInformation.find(virtualAddress);
  if (it == mallocInformation.end())
    return;

  statBytesFree[it->second.level]->addData(it->second.size);

  // Free each VA in mallocInformation from mallocTranslations &
  // mallocPrimaryVAMap TODO fix so that mapping stays but address is available
  // for future mallocs
  std::unordered_set<uint64_t> *myKeys = (it->second.VAKeys);
  for (unsigned long myKey : *myKeys) {
    mallocPrimaryVAMap.erase(myKey);
    freePages[(it->second).level]->push_front(
        mallocTranslations.find(myKey)->second);
    mallocTranslations.erase(myKey);
  }
...
uint64_t ArielMemoryManagerMalloc::translateAddress(uint64_t virtAddr) {
  ...
  auto physAddr = (uint64_t)-1;
  bool found = false;

  output->verbose(CALL_INFO, 4, 0,
                  "Page Table: translate virtual address %" PRIu64 "\n",
                  virtAddr);
  ...
  if (!mallocTranslations.empty()) {
    auto it = mallocTranslations.upper_bound(virtAddr);
    if (it == mallocTranslations.begin())
      it = mallocTranslations.end();
    else
      it--;
  ...
  for (uint32_t i = 0; i < memoryLevels; ++i) {
    if (!found) {
      ...
      if (page_itr != pageTables[i]->end()) {
        ...
        output->verbose(
            CALL_INFO, 4, 0,
            "Page table hit: virtual address=%" PRIu64 " hit in level: %" PRIu32
            ", virtual page start=%" PRIu64 ", virtual end=%" PRIu64
            ", translates to phys page start=%" PRIu64
            " translates to: phys address: %" PRIu64
            " (offset added to phys start=%" PRIu64 ")\n",
            virtAddr, i, page_itr->first, page_itr->first + pageSize,
            page_itr->second, physAddr, page_offset);
  ...
  if (found) {
    ...
    output->verbose(CALL_INFO, 4, 0,
                    "Page table miss for virtual address: %" PRIu64 "\n",
                    virtAddr);

    // We did not find the address in memory, that means we should allocate it
    // one from our default pool
    uint64_t offset = virtAddr % pageSizes[defaultLevel];

    output->verbose(CALL_INFO, 4, 0,
                    "Page offset calculation (generating a new page allocation "
                    "request) for address %" PRIu64 ", offset=%" PRIu64
                    ", requesting virtual map to address: %" PRIu64 "\n",
    ...
    if (canAllocateInLevel(8, defaultLevel)) {
      ...
      if (!allocated)
        output->fatal(CALL_INFO, -1,
                      "Attempted to allocate page for address %" PRIu64
    ...
    output->verbose(CALL_INFO, 4, 0,
                    "Page allocation routine mapped to address: %" PRIu64 "\n",
                    newPhysAddr);
...
void ArielMemoryManagerMalloc::printStats() {
  ...
  for (uint32_t i = 0; i < memoryLevels; ++i) {
    output->output(
        "- Demand bytes at level %" PRIu32 "              %" PRIu64 "\n", i,
        ((uint64_t)pageTables[i]->size()) * ((uint64_t)pageSizes[i]));
  }
}
