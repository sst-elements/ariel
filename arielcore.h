...
public:
  ...
  void createAllocateEvent(uint64_t vAddr, uint64_t length, uint32_t level,
                           uint64_t ip);
  void createMmapEvent(uint32_t fileID, uint64_t vAddr, uint64_t length,
  ...
  void commitReadEvent(const uint64_t address, const uint64_t virtAddr,
                       const uint32_t length);
  void commitWriteEvent(const uint64_t address, const uint64_t virtAddr,
                        const uint32_t length, const uint8_t *payload);
  void commitFlushEvent(const uint64_t address, const uint64_t virtAddr,
                        const uint32_t length);
  ...
  void printTraceEntry(const bool isRead, const uint64_t address,
                       const uint32_t length);
...
private:
  ...
  std::unordered_map<SimpleMem::Request::id_t, SimpleMem::Request *>
