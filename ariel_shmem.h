 ...
 *  - Only PinCRT-enabled includes are allowed (no PinCRT specific ones and no
 * non-enabled ones)
 */
...
class ArielTunnel
    : public SST::Core::Interprocess::TunnelDef<ArielSharedData, ArielCommand> {
public:
  /**
   * Create a new Ariel Tunnel
   */
  ArielTunnel(size_t numCores, size_t bufferSize, uint32_t expectedChildren = 1)
      : SST::Core::Interprocess::TunnelDef<ArielSharedData, ArielCommand>(
            numCores, bufferSize, expectedChildren) {}
  ...
  /**
   * Initialize tunnel
   * None of the data structures (e.g., sharedData) are available until this
   * function call
   */
  virtual uint32_t initialize(void *sPtr) {
    uint32_t childnum =
        SST::Core::Interprocess::TunnelDef<ArielSharedData,
                                           ArielCommand>::initialize(sPtr);
    if (isMaster()) {
  ...
  void updateTime(uint64_t newTime) { sharedData->simTime = newTime; }

  /** Increment current cycle count */
  void incrementCycles() { sharedData->cycles++; }

  uint64_t getCycles() const { return sharedData->cycles; }
...
class GpuReturnTunnel
    : public SST::Core::Interprocess::TunnelDef<GpuSharedData, GpuCommand> {
public:
  /**
   * Create a new Gpu Tunnel
   */
  GpuReturnTunnel(size_t numCores, size_t bufferSize,
                  uint32_t expectedChildren = 1)
      : SST::Core::Interprocess::TunnelDef<GpuSharedData, GpuCommand>(
            numCores, bufferSize, expectedChildren) {}
  ...
  virtual uint32_t initialize(void *sPtr) {
    uint32_t childnum =
        SST::Core::Interprocess::TunnelDef<GpuSharedData,
                                           GpuCommand>::initialize(sPtr);
    if (isMaster()) {
      sharedData->numCores = getNumBuffers();
      sharedData->child_attached = 0;
    } else {
...
class GpuDataTunnel
    : public SST::Core::Interprocess::TunnelDef<GpuSharedData, GpuDataCommand> {
public:
  /**
   * Create a new Gpu Tunnel
   */
  GpuDataTunnel(size_t numCores, size_t bufferSize,
                uint32_t expectedChildren = 1)
      : SST::Core::Interprocess::TunnelDef<GpuSharedData, GpuDataCommand>(
            numCores, bufferSize, expectedChildren) {}
  ...
  virtual uint32_t initialize(void *sPtr) {
    uint32_t childnum =
        SST::Core::Interprocess::TunnelDef<GpuSharedData,
                                           GpuDataCommand>::initialize(sPtr);
    if (isMaster()) {
      sharedData->numCores = getNumBuffers();
      sharedData->child_attached = 0;
    } else {
